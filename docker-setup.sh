...
if [[ -z "${OPENCLAW_GATEWAY_TOKEN:-}" ]]; then
  ...
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    ...
    FORGE_ORCH_GATEWAY_TOKEN="$(python3 - <<'PY'
import secrets
...
export FORGE_ORCH_GATEWAY_TOKEN
...
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm forge-orchestrator-cli channels login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm forge-orchestrator-cli channels add --channel telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm forge-orchestrator-cli channels add --channel discord --token <token>"
echo "Docs: https://docs.forge-orchestrator.ai/channels"

echo ""
echo "==> Starting gateway"
docker compose "${COMPOSE_ARGS[@]}" up -d forge-orchestrator-gateway
...
echo "Config: $FORGE_ORCH_CONFIG_DIR"
echo "Workspace: $FORGE_ORCH_WORKSPACE_DIR"
echo "Token: $FORGE_ORCH_GATEWAY_TOKEN"
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f forge-orchestrator-gateway"
echo "  ${COMPOSE_HINT} exec forge-orchestrator-gateway node dist/index.js health --token \"$FORGE_ORCH_GATEWAY_TOKEN\""
