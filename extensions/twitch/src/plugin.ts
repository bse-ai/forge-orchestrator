/**
 * Twitch channel plugin for ForgeOrchestrator.
 *
 ...
 * for ForgeOrchestrator. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: ForgeOrchestratorConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: ForgeOrchestratorConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: ForgeOrchestratorConfig): boolean => {
