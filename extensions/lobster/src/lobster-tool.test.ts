...
function fakeApi(overrides: Partial<ForgeOrchestratorPluginApi> = {}): ForgeOrchestratorPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<ForgeOrchestratorPluginToolContext> = {}): ForgeOrchestratorPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: ForgeOrchestratorPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
