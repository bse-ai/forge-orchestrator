...
vi.mock("forge-orchestrator/plugin-sdk", async () => {
  const actual = await vi.importActual<typeof import("forge-orchestrator/plugin-sdk")>("forge-orchestrator/plugin-sdk");
  return {
    ...actual,
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("forge-orchestrator.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("forge-orchestrator.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("forge-orchestrator.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("forge-orchestrator.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("forge-orchestrator.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("forge-orchestrator.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("forge-orchestrator.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("forge-orchestrator.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("forge-orchestrator.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("forge-orchestrator.webhook.processed");
    expect(spanNames).toContain("forge-orchestrator.message.processed");
    expect(spanNames).toContain("forge-orchestrator.session.stuck");
