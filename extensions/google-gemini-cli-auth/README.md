# Google Gemini CLI Auth (ForgeOrchestrator plugin)
...
```bash
forge-orchestrator plugins enable google-gemini-cli-auth
```
...
```bash
forge-orchestrator models auth login --provider google-gemini-cli --set-default
```
...
- `FORGE_ORCH_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `FORGE_ORCH_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
