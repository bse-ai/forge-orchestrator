...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.FORGE_ORCH_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-nostr-"));
  process.env.FORGE_ORCH_STATE_DIR = dir;
  setNostrRuntime({
    state: {
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.FORGE_ORCH_STATE_DIR = previous;
    }
