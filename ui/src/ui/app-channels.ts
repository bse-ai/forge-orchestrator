...
export async function handleWhatsAppStart(host: ForgeOrchestratorApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: ForgeOrchestratorApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: ForgeOrchestratorApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: ForgeOrchestratorApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: ForgeOrchestratorApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: ForgeOrchestratorApp): string {
...
function resolveGatewayHttpAuthHeader(host: ForgeOrchestratorApp): string | null {
...
function buildGatewayHttpHeaders(host: ForgeOrchestratorApp): Record<string, string> {
  const authorization = resolveGatewayHttpAuthHeader(host);
  return authorization ? { Authorization: authorization } : {};
}

export function handleNostrProfileEdit(
  host: ForgeOrchestratorApp,
  accountId: string,
...
export function handleNostrProfileCancel(host: ForgeOrchestratorApp) {
...
export function handleNostrProfileToggleAdvanced(host: ForgeOrchestratorApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: ForgeOrchestratorApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: ForgeOrchestratorApp) {
  const state = host.nostrProfileFormState;
