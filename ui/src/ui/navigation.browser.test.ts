...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/forge-orchestrator/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/forge-orchestrator");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/forge-orchestrator/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__FORGE_ORCH_CONTROL_UI_BASE_PATH__ = "/forge-orchestrator";
    const app = mountApp("/forge-orchestrator/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/forge-orchestrator");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/forge-orchestrator/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem(
      "forge-orchestrator.control.settings.v1",
