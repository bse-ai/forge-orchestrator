...
- Plugin manifests (`forge-orchestrator.plugin.json`) are required for all plugins.

## Plugin schema enforcement

- Each plugin provides a strict JSON Schema for its config (inline in the manifest).
- Plugin load flow:
  1. Resolve plugin manifest + schema (`forge-orchestrator.plugin.json`).
...
- If config invalid:
  ...
  - Instruct: `forge-orchestrator doctor --fix`.
- `forge-orchestrator doctor --fix`:
  - Applies migrations.
...
- `forge-orchestrator help`
- `forge-orchestrator status`
- `forge-orchestrator gateway status`

Everything else must hard-fail with: “Config invalid. Run `forge-orchestrator doctor --fix`.”
