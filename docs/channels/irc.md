...
Use IRC when you want ForgeOrchestrator in classic channels (`#room`) and direct messages.
IRC ships as an extension plugin, but it is configured in the main config under `channels.irc`.

## Quick start

1. Enable IRC config in `~/.forge-orchestrator/forge-orchestrator.json`.
2. Set at least:
...
```json
{
  "channels": {
    "irc": {
      ...
      "port": 6697,
      "tls": true,
      "nick": "forge-orchestrator-bot",
      "channels": ["#forge-orchestrator"]
    }
  }
}
```
...
Even if a channel is allowed (via `groupPolicy` + `groups`) and the sender is allowed, ForgeOrchestrator defaults to **mention-gating** in group contexts.
