...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects ForgeOrchestrator to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
forge-orchestrator plugins install @forge-orchestrator/feishu
```

Local checkout (when running from a git repo):

```bash
forge-orchestrator plugins install ./extensions/feishu
```
...
If you just installed ForgeOrchestrator, run the wizard:
...
2. Configuring app credentials in ForgeOrchestrator
...
```bash
forge-orchestrator channels add
```
...
1. You already ran `forge-orchestrator channels add` for Feishu
2. The gateway is running (`forge-orchestrator gateway status`)
...
## Step 2: Configure ForgeOrchestrator
...
Edit `~/.forge-orchestrator/forge-orchestrator.json`:
...
```bash
forge-orchestrator pairing approve feishu <CODE>
```
...
- **Approve pairing**:
  ...
  ```bash
  forge-orchestrator pairing list feishu
  forge-orchestrator pairing approve feishu <CODE>
  ```
...
2. Run `forge-orchestrator logs --follow` and look for `chat_id`
...
2. Run `forge-orchestrator logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `forge-orchestrator gateway status`  | Show gateway status           |
| `forge-orchestrator gateway install` | Install/start gateway service |
| `forge-orchestrator gateway stop`    | Stop gateway service          |
| `forge-orchestrator gateway restart` | Restart gateway service       |
| `forge-orchestrator logs --follow`   | Tail gateway logs             |
...
4. Check logs: `forge-orchestrator logs --follow`
...
5. Ensure the gateway is running: `forge-orchestrator gateway status`
6. Check logs: `forge-orchestrator logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.forge-orchestrator/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.forge-orchestrator/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
