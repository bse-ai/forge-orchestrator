...
ForgeOrchestrator routes replies **back to the channel where a message came from**. The
...
Broadcast groups let you run **multiple agents** for the same peer **when ForgeOrchestrator would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.forge-orchestrator/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.forge-orchestrator`):

- `~/.forge-orchestrator/agents/<agentId>/sessions/sessions.json`
