...
read_when:
  - Setting up Zalo Personal for ForgeOrchestrator
...
- Install via CLI: `forge-orchestrator plugins install @forge-orchestrator/zalouser`
- Or from a source checkout: `forge-orchestrator plugins install ./extensions/zalouser`
...
```bash
forge-orchestrator directory self --channel zalouser
forge-orchestrator directory peers list --channel zalouser --query "name"
forge-orchestrator directory groups list --channel zalouser --query "work"
```
...
- `forge-orchestrator pairing list zalouser`
- `forge-orchestrator pairing approve zalouser <code>`
...
- On startup, ForgeOrchestrator resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
