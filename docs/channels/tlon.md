...
Tlon is a decentralized messenger built on Urbit. ForgeOrchestrator connects to your Urbit ship and can
...
```bash
forge-orchestrator plugins install @forge-orchestrator/tlon
```

Local checkout (when running from a git repo):

```bash
forge-orchestrator plugins install ./extensions/tlon
```
...
Use these with `forge-orchestrator message send` or cron delivery:
