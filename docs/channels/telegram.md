...
<Tabs>
  <Tab title="DM policy">
    ...
    1. DM your bot.
    2. Run `forge-orchestrator logs --follow`.
    3. Read `from.id`.
...
```
    ...
    - or read `chat.id` from `forge-orchestrator logs --follow`
...
- DM messages can carry `message_thread_id`; ForgeOrchestrator routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, ForgeOrchestrator retries as plain text.
...
```
    ...
    - `~/.forge-orchestrator/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, ForgeOrchestrator enqueues system events like:
...
```bash
forge-orchestrator message send --channel telegram --target 123456789 --message "hi"
forge-orchestrator message send --channel telegram --target @name --message "hi"
```
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `forge-orchestrator channels status` warns when config expects unmentioned group messages.
    - `forge-orchestrator channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `forge-orchestrator logs --follow` for skip reasons
