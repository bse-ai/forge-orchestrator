---
summary: "Mattermost bot setup and ForgeOrchestrator config"
read_when:
...
```bash
forge-orchestrator plugins install @forge-orchestrator/mattermost
```

Local checkout (when running from a git repo):

```bash
forge-orchestrator plugins install ./extensions/mattermost
```
...
4. Configure ForgeOrchestrator and start the gateway.
...
- Approve via:
  - `forge-orchestrator pairing list mattermost`
  - `forge-orchestrator pairing approve mattermost <CODE>`
...
Use these target formats with `forge-orchestrator message send` or cron/webhooks:
