...
```bash
forge-orchestrator plugins install @forge-orchestrator/msteams
```

Local checkout (when running from a git repo):

```bash
forge-orchestrator plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
ForgeOrchestrator will offer the local install path automatically.
...
3. Configure ForgeOrchestrator with those credentials.
...
- Talk to ForgeOrchestrator via Teams DMs, group chats, or channels.
...
- On startup, ForgeOrchestrator resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.forge-orchestrator/forge-orchestrator.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring ForgeOrchestrator, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `forge-orchestrator-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `forge-orchestrator plugins install @forge-orchestrator/msteams`
   - From a local checkout: `forge-orchestrator plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "ForgeOrchestrator in Teams", "full": "ForgeOrchestrator in Teams" },
...
ForgeOrchestrator handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, ForgeOrchestrator only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/ForgeOrchestratorShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

ForgeOrchestrator sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `forge-orchestrator message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.forge-orchestrator/msteams-polls.json`.
...
```bash
forge-orchestrator message send --channel msteams \
...
forge-orchestrator message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
forge-orchestrator message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
forge-orchestrator message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
forge-orchestrator message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
