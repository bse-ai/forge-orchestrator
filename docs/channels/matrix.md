...
Matrix is an open, decentralized messaging protocol. ForgeOrchestrator connects as a Matrix **user**
...
```bash
forge-orchestrator plugins install @forge-orchestrator/matrix
```

Local checkout (when running from a git repo):

```bash
forge-orchestrator plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `forge-orchestrator plugins install @forge-orchestrator/matrix`
   - From a local checkout: `forge-orchestrator plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: ForgeOrchestrator calls the same
     login endpoint, stores the access token in `~/.forge-orchestrator/credentials/matrix/credentials.json`,
...
- On first connection, ForgeOrchestrator requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  ForgeOrchestrator logs a warning.
...
`~/.forge-orchestrator/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `forge-orchestrator pairing list matrix`
  - `forge-orchestrator pairing approve matrix <CODE>`
...
- On startup, ForgeOrchestrator resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
