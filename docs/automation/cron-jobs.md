...
- Jobs persist under `~/.forge-orchestrator/cron/` so restarts don’t lose schedules.
...
forge-orchestrator cron run <job-id>
forge-orchestrator cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.forge-orchestrator/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `forge-orchestrator cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, ForgeOrchestrator defaults to `announce`.
...
```json5
{
  cron: {
    ...
    store: "~/.forge-orchestrator/cron/jobs.json",
...
forge-orchestrator cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
forge-orchestrator cron edit <jobId> --agent ops
forge-orchestrator cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
forge-orchestrator cron run <jobId>
forge-orchestrator cron run <jobId> --due
```
...
```bash
forge-orchestrator cron edit <jobId> \
...
```bash
forge-orchestrator system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`forge-orchestrator system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `FORGE_ORCH_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- ForgeOrchestrator applies exponential retry backoff for recurring jobs after consecutive errors:
