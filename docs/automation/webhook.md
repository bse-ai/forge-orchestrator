...
read_when:
  ...
  - Wiring external systems into ForgeOrchestrator
title: "Webhooks"
---
...
- `x-forge-orchestrator-token: <token>`
...
- `forge-orchestrator webhooks gmail setup` writes `hooks.gmail` config for `forge-orchestrator webhooks gmail run`.
...
curl -X POST http://127.0.0.1:18789/hooks/agent \
  -H 'x-forge-orchestrator-token: SECRET' \
  -H 'Content-Type: application/json' \
