---
summary: "ForgeOrchestrator macOS companion app (menu bar + gateway broker)"
read_when:
...
# ForgeOrchestrator macOS Companion (menu bar + gateway broker)

The macOS app is the **menu‑bar companion** for ForgeOrchestrator. It owns permissions,
...
- Installs the global CLI (`forge-orchestrator`) via npm/pnpm on request (bun not recommended for the Gateway runtime).

## Local vs remote mode

- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via `forge-orchestrator gateway install`.
...
```
~/.forge-orchestrator/exec-approvals.json
```
...
The app registers the `forge-orchestrator://` URL scheme for local actions.
...
```bash
open 'forge-orchestrator://agent?message=Hello%20from%20deep%20link'
```
...
1. Install and launch **ForgeOrchestrator.app**.
...
- `swift run ForgeOrchestrator` (or Xcode)
...
```bash
cd apps/macos
swift run forge-orchestrator-mac connect --json
swift run forge-orchestrator-mac discover --timeout 3000 --json
```
...
Tip: compare against `forge-orchestrator gateway discover --json` to see whether the
