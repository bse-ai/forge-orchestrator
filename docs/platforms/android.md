...
- You can run the CLI (`forge-orchestrator`) on the gateway machine (or via SSH).
...
- Set `gateway.bind: "tailnet"` in `~/.forge-orchestrator/forge-orchestrator.json` on the gateway host.
...
1. Set up a DNS-SD zone (example `forge-orchestrator.internal.`) on the gateway host and publish `_forge-orchestrator-gw._tcp` records.
...
1. Create `~/.forge-orchestrator/workspace/canvas/index.html` on the gateway host.
