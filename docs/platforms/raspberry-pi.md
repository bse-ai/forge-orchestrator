---
summary: "ForgeOrchestrator on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up ForgeOrchestrator on a Raspberry Pi
  - Running ForgeOrchestrator on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# ForgeOrchestrator on Raspberry Pi

## Goal

Run a persistent, always-on ForgeOrchestrator Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# Check service
sudo systemctl status forge-orchestrator

# View logs
journalctl -u forge-orchestrator -f
```
...
sudo systemctl restart forge-orchestrator
```
...
Most ForgeOrchestrator features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled forge-orchestrator

# Enable if not
sudo systemctl enable forge-orchestrator

# Start on boot
sudo systemctl start forge-orchestrator
```
...
```bash
# Check logs
journalctl -u forge-orchestrator --no-pager -n 100

# Common fix: rebuild
cd ~/forge-orchestrator  # if using hackable install
npm run build
sudo systemctl restart forge-orchestrator
```
