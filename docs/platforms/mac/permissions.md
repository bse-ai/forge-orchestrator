...
- Same path: run the app from a fixed location (for ForgeOrchestrator, `dist/ForgeOrchestrator.app`).
...
Workaround: move files into the ForgeOrchestrator workspace (`~/.forge-orchestrator/workspace`) if you want to avoid per-folder grants.
