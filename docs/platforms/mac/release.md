---
summary: "ForgeOrchestrator macOS release checklist (Sparkle feed, packaging, signing)"
read_when:
  - Cutting or validating a ForgeOrchestrator macOS release
  - Updating the Sparkle appcast or feed assets
title: "macOS Release"
---

# ForgeOrchestrator macOS release (Sparkle)
...
- Notary credentials (keychain profile or API key) for `xcrun notarytool` if you want Gatekeeper-safe DMG/zip distribution.
  - We use a Keychain profile named `forge-orchestrator-notary`, created from App Store Connect API key env vars in your shell profile:
    - `APP_STORE_CONNECT_API_KEY_P8`, `APP_STORE_CONNECT_KEY_ID`, `APP_STORE_CONNECT_ISSUER_ID`
    - `echo "$APP_STORE_CONNECT_API_KEY_P8" | sed 's/\\n/\n/g' > /tmp/forge-orchestrator-notary.p8`
    - `xcrun notarytool store-credentials "forge-orchestrator-notary" --key /tmp/forge-orchestrator-notary.p8 --key-id "$APP_STORE_CONNECT_KEY_ID" --issuer "$APP_STORE_CONNECT_ISSUER_ID"`
...
#   xcrun notarytool store-credentials "forge-orchestrator-notary" \
...
Generates HTML release notes from `CHANGELOG.md` (via [`scripts/changelog-to-html.sh`](https://github.com/forge-orchestrator/forge-orchestrator/blob/main/scripts/changelog-to-html.sh)) and embeds them in the appcast entry.
...
- Sanity checks:
  - `curl -I https://raw.githubusercontent.com/forge-orchestrator/forge-orchestrator/main/appcast.xml` returns 200.
