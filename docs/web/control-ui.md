...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/forge-orchestrator`)
...
If the page fails to load, start the Gateway first: `forge-orchestrator gateway`.
...
you revoke it with `forge-orchestrator devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.forge-orchestrator/forge-orchestrator.json` (`config.get`, `config.set`)
...
```bash
forge-orchestrator gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
ForgeOrchestrator **blocks** Control UI connections without device identity.
...
```bash
FORGE_ORCH_CONTROL_UI_BASE_PATH=/forge-orchestrator/ pnpm ui:build
```
