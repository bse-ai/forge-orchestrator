---
summary: "Optional Docker-based setup and onboarding for ForgeOrchestrator"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run ForgeOrchestrator on a host without local installs.
- **No**: you’re running on your own machine and just want the fastest dev loop. Use the normal install flow instead.
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. See [Sandboxing](/gateway/sandboxing).

This guide covers:

- Containerized Gateway (full ForgeOrchestrator in Docker)
...
- Need the URL again? Run `docker compose run --rm forge-orchestrator-cli dashboard --no-open`.
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/forge-orchestrator/forge-orchestrator/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
```
...
See [`ClawDock` Helper README](https://github.com/forge-orchestrator/forge-orchestrator/blob/main/scripts/shell-helpers/README.md) for details.

### Manual flow (compose)

```bash
docker build -t forge-orchestrator:local -f Dockerfile .
docker compose run --rm forge-orchestrator-cli onboard
docker compose up -d forge-orchestrator-gateway
```

Note: run `docker compose ...` from the repo root. If you enabled
`FORGE_ORCH_EXTRA_MOUNTS` or `FORGE_ORCH_HOME_VOLUME`, the setup script writes
...
```bash
docker compose run --rm forge-orchestrator-cli dashboard --no-open
docker compose run --rm forge-orchestrator-cli devices list
docker compose run --rm forge-orchestrator-cli devices approve <requestId>
```
...
`FORGE_ORCH_EXTRA_MOUNTS` before running `docker-setup.sh`. This accepts a
comma-separated list of Docker bind mounts and applies them to both
`forge-orchestrator-gateway` and `forge-orchestrator-cli` by generating `docker-compose.extra.yml`.

Example:

```bash
export FORGE_ORCH_EXTRA_MOUNTS="$HOME/.codex:/home/node/.codex:ro,$HOME/github:/home/node/github:rw"
./docker-setup.sh
```
...
volume via `FORGE_ORCH_HOME_VOLUME`. This creates a Docker volume and mounts it at
...
```bash
export FORGE_ORCH_HOME_VOLUME="forge-orchestrator_home"
./docker-setup.sh
```
...
libraries), set `FORGE_ORCH_DOCKER_APT_PACKAGES` before running `docker-setup.sh`.
...
```bash
export FORGE_ORCH_DOCKER_APT_PACKAGES="ffmpeg build-essential"
./docker-setup.sh
```
...
- If you change `FORGE_ORCH_DOCKER_APT_PACKAGES`, rerun `docker-setup.sh` to rebuild
  the image.
...
```bash
export FORGE_ORCH_DOCKER_APT_PACKAGES="git curl jq"
./docker-setup.sh
```
...
`FORGE_ORCH_DOCKER_APT_PACKAGES` instead of using `--with-deps` at runtime.

4. **Persist Playwright browser downloads**:

- Set `PLAYWRIGHT_BROWSERS_PATH=/home/node/.cache/ms-playwright` in
  `docker-compose.yml`.
- Ensure `/home/node` persists via `FORGE_ORCH_HOME_VOLUME`, or mount
  `/home/node/.cache/ms-playwright` via `FORGE_ORCH_EXTRA_MOUNTS`.

### Permissions + EACCES

The image runs as `node` (uid 1000). If you see permission errors on
`/home/node/.forge-orchestrator`, make sure your host bind mounts are owned by uid 1000.

Example (Linux host):

```bash
sudo chown -R 1000:1000 /path/to/forge-orchestrator-config /path/to/forge-orchestrator-workspace
```
...
```bash
docker compose run --rm forge-orchestrator-cli channels login
```

Telegram (bot token):

```bash
docker compose run --rm forge-orchestrator-cli channels add --channel telegram --token "<token>"
```

Discord (bot token):

```bash
docker compose run --rm forge-orchestrator-cli channels add --channel discord --token "<token>"
```
...
```bash
docker compose exec forge-orchestrator-gateway node dist/index.js health --token "$FORGE_ORCH_GATEWAY_TOKEN"
```
...
- The gateway container is the source of truth for sessions (`~/.forge-orchestrator/agents/<agentId>/sessions/`).
...
- Agent workspace access: `workspaceAccess: "none"` (default) uses `~/.forge-orchestrator/sandboxes`
...
- `user` must be root for `apt-get` (omit `user` or set `user: "0:0"`).
  ForgeOrchestrator auto-recreates containers when `setupCommand` (or docker config) changes
  unless the container was **recently used** (within ~5 minutes). Hot containers
  log a warning with the exact `forge-orchestrator sandbox recreate ...` command.
...
This builds `forge-orchestrator-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
This builds `forge-orchestrator-sandbox-common:bookworm-slim`. To use it:

```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "forge-orchestrator-sandbox-common:bookworm-slim" } },
    },
  },
}
```
...
This builds `forge-orchestrator-sandbox-browser:bookworm-slim` using
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my-forge-orchestrator-browser" } },
    },
  },
}
```
...
```bash
docker build -t my-forge-orchestrator-sbx -f Dockerfile.sandbox .
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "my-forge-orchestrator-sbx" } },
    },
  },
}
```
...
- Image missing: build with [`scripts/sandbox-setup.sh`](https://github.com/forge-orchestrator/forge-orchestrator/blob/main/scripts/sandbox-setup.sh) or set `agents.defaults.sandbox.docker.image`.
- Container not running: it will auto-create per session on demand.
- Permission errors in sandbox: set `docker.user` to a UID:GID that matches your
  mounted workspace ownership (or chown the workspace folder).
- Custom tools not found: ForgeOrchestrator runs commands with `sh -lc` (login shell), which
