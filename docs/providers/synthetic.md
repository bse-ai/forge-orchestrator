---
summary: "Use Synthetic's Anthropic-compatible API in ForgeOrchestrator"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. ForgeOrchestrator registers it as the
...
```bash
forge-orchestrator onboard --auth-choice synthetic-api-key
```
...
Note: ForgeOrchestrator's Anthropic client appends `/v1` to the base URL, so use
