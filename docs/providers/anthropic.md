---
summary: "Use Anthropic Claude via API keys or setup-token in ForgeOrchestrator"
read_when:
  - You want to use Anthropic models in ForgeOrchestrator
...
In ForgeOrchestrator you can authenticate with an API key or a **setup-token**.
...
# or non-interactive
forge-orchestrator onboard --anthropic-api-key "$ANTHROPIC_API_KEY"
```
...
ForgeOrchestrator supports Anthropic's prompt caching feature. This is **API-only**; subscription auth does not honor cache settings.
...
When using Anthropic API Key authentication, ForgeOrchestrator automatically applies `cacheRetention: "short"` (5-minute cache) for all Anthropic models. You can override this by explicitly setting `cacheRetention` in your config.
...
ForgeOrchestrator includes the `extended-cache-ttl-2025-04-11` beta flag for Anthropic API
...
Paste the token into ForgeOrchestrator (wizard: **Anthropic token (paste setup-token)**), or run it on the gateway host:
...
```bash
forge-orchestrator models auth paste-token --provider anthropic
```
...
forge-orchestrator onboard --auth-choice setup-token
```
...
- Generate the setup-token with `claude setup-token` and paste it, or run `forge-orchestrator models auth setup-token` on the gateway host.
...
- If the Claude CLI login lives on a different machine, use
  `forge-orchestrator models auth paste-token --provider anthropic` on the gateway host.

**No API key found for provider "anthropic"**

- Auth is **per agent**. New agents don’t inherit the main agent’s keys.
- Re-run onboarding for that agent, or paste a setup-token / API key on the
  gateway host, then verify with `forge-orchestrator models status`.

**No credentials found for profile `anthropic:default`**

- Run `forge-orchestrator models status` to see which auth profile is active.
- Re-run onboarding, or paste a setup-token / API key for that profile.

**No available auth profile (all in cooldown/unavailable)**

- Check `forge-orchestrator models status --json` for `auth.unusableProfiles`.
