...
read_when:
  - You want MiniMax models in ForgeOrchestrator
...
```bash
forge-orchestrator plugins enable minimax-portal-auth  # skip if already loaded.
forge-orchestrator gateway restart  # restart if gateway is already running
forge-orchestrator onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/forge-orchestrator/forge-orchestrator/tree/main/extensions/minimax-portal-auth) for details.
...
- Run `forge-orchestrator configure`
- Select **Model/auth**
- Choose **MiniMax M2.5**
...
Configure manually via `forge-orchestrator.json`:
