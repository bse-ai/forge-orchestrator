---
summary: "Use Venice AI privacy-focused models in ForgeOrchestrator"
read_when:
  - You want privacy-focused inference in ForgeOrchestrator
...
## Why Venice in ForgeOrchestrator
...
```bash
forge-orchestrator onboard --auth-choice venice-api-key
```
...
After setup, ForgeOrchestrator shows all available Venice models. Pick based on your needs:
...
```bash
forge-orchestrator models set venice/claude-opus-45
forge-orchestrator models set venice/llama-3.3-70b
```

List all available models:

```bash
forge-orchestrator models list | grep venice
```

## Configure via `forge-orchestrator configure`

1. Run `forge-orchestrator configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
ForgeOrchestrator automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
The Venice model catalog updates dynamically. Run `forge-orchestrator models list` to see currently available models. Some models may be temporarily offline.
