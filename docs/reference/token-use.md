---
summary: "How ForgeOrchestrator builds prompt context and reports token usage + costs"
read_when:
...
ForgeOrchestrator tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

ForgeOrchestrator assembles its own system prompt on every run. It includes:
...
- `/usage cost` → shows a local cost summary from ForgeOrchestrator session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `forge-orchestrator status --usage` and `forge-orchestrator channels list` show
...
`cacheWrite`. If pricing is missing, ForgeOrchestrator shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. ForgeOrchestrator can
