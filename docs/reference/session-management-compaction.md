...
This document explains how ForgeOrchestrator manages sessions end-to-end:
...
ForgeOrchestrator is designed around a single **Gateway process** that owns session state.
...
ForgeOrchestrator persists sessions in two layers:
...
- Transcripts: `~/.forge-orchestrator/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

ForgeOrchestrator resolves these via `src/config/sessions.ts`.
...
ForgeOrchestrator intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (ForgeOrchestrator consumes the events, but Pi decides when to compact).
...
ForgeOrchestrator also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, ForgeOrchestrator bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, ForgeOrchestrator leaves it alone.
...
- `forge-orchestrator status` (CLI)
- `forge-orchestrator sessions` / `sessions --json`
...
ForgeOrchestrator supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- ForgeOrchestrator strips/suppresses this in the delivery layer.

As of `2026.1.10`, ForgeOrchestrator also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
ForgeOrchestrator uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but ForgeOrchestrator’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `forge-orchestrator status`.
