...
ForgeOrchestrator features that can generate provider usage or paid API calls.
...
- `forge-orchestrator status --usage` and `forge-orchestrator channels list` show provider **usage windows**
...
ForgeOrchestrator can pick up credentials from:
...
- `forge-orchestrator status --usage`
- `forge-orchestrator models status --json`
...
`forge-orchestrator models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
