---
read_when:
  - 新規ユーザーにForgeOrchestratorを紹介するとき
summary: ForgeOrchestratorは、あらゆるOSで動作するAIエージェント向けのマルチチャネルgatewayです。
...
<Columns>
  <Card title="はじめに" href="/start/getting-started" icon="rocket">
    ForgeOrchestratorをインストールし、数分でGatewayを起動できます。
  </Card>
  <Card title="ウィザードを実行" href="/start/wizard" icon="sparkles">
    `forge-orchestrator onboard`とペアリングフローによるガイド付きセットアップ。
  </Card>
...
ForgeOrchestratorは、単一のGatewayプロセスを通じてチャットアプリをPiのようなコーディングエージェントに接続します。ForgeOrchestratorアシスタントを駆動し、ローカルまたはリモートのセットアップをサポートします。
...
<Steps>
  <Step title="ForgeOrchestratorをインストール">
    ```bash
...
設定は`~/.forge-orchestrator/forge-orchestrator.json`にあります。

- **何もしなければ**、ForgeOrchestratorはバンドルされたPiバイナリをRPCモードで使用し、送信者ごとのセッションを作成します。
