...
- [Quick start and first-run setup]
  ...
  - [I can't access docs.forge-orchestrator.ai (SSL error). What now?](#i-cant-access-docsforge-orchestratorai-ssl-error-what-now)
  ...
  - [How do I install ForgeOrchestrator on Linux?](#how-do-i-install-forge-orchestrator-on-linux)
  - [How do I install ForgeOrchestrator on a VPS?](#how-do-i-install-forge-orchestrator-on-a-vps)
  - [Where are the cloud/VPS install guides?](#where-are-the-cloudvps-install-guides)
  - [Can I ask ForgeOrchestrator to update itself?](#can-i-ask-forge-orchestrator-to-update-itself)
  ...
  - [If I buy a Mac mini to run ForgeOrchestrator, can I connect it to my MacBook Pro?](#if-i-buy-a-mac-mini-to-run-forge-orchestrator-can-i-connect-it-to-my-macbook-pro)
  - [Can I use Bun?](#can-i-use-bun)
  - [Telegram: what goes in `allowFrom`?](#telegram-what-goes-in-allowfrom)
  - [Can multiple people use one WhatsApp number with different ForgeOrchestrator instances?](#can-multiple-people-use-one-whatsapp-number-with-different-forge-orchestrator-instances)
  - [Can I run a "fast chat" agent and an "Opus for coding" agent?](#can-i-run-a-fast-chat-agent-and-an-opus-for-coding-agent)
  - [Does Homebrew work on Linux?](#does-homebrew-work-on-linux)
  - [What's the difference between the hackable (git) install and npm install?](#whats-the-difference-between-the-hackable-git-install-and-npm-install)
  - [Can I switch between npm and git installs later?](#can-i-switch-between-npm-and-git-installs-later)
  - [Should I run the Gateway on my laptop or a VPS?](#should-i-run-the-gateway-on-my-laptop-or-a-vps)
  - [How important is it to run ForgeOrchestrator on a dedicated machine?](#how-important-is-it-to-run-forge-orchestrator-on-a-dedicated-machine)
  - [What are the minimum VPS requirements and recommended OS?](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [Can I run ForgeOrchestrator in a VM and what are the requirements](#can-i-run-forge-orchestrator-in-a-vm-and-what-are-the-requirements)
- [What is ForgeOrchestrator?](#what-is-forge-orchestrator)
  - [What is ForgeOrchestrator, in one paragraph?](#what-is-forge-orchestrator-in-one-paragraph)
  - [What's the value proposition?](#whats-the-value-proposition)
  - [I just set it up what should I do first](#i-just-set-it-up-what-should-i-do-first)
  - [What are the top five everyday use cases for ForgeOrchestrator](#what-are-the-top-five-everyday-use-cases-for-forge-orchestrator)
  - [Can ForgeOrchestrator help with lead gen outreach ads and blogs for a SaaS](#can-forge-orchestrator-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills and automation](#skills-and-automation)
  ...
  - [Can ForgeOrchestrator run tasks on a schedule or continuously in the background?](#can-forge-orchestrator-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [Where things live on disk](#where-things-live-on-disk)
  - [Is all data used with ForgeOrchestrator saved locally?](#is-all-data-used-with-forge-orchestrator-saved-locally)
  - [Where does ForgeOrchestrator store its data?](#where-does-forge-orchestrator-store-its-data)
  - [Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?](#where-should-agentsmd-soulmd-usermd-memorymd-live)
  - [What's the recommended backup strategy?](#whats-the-recommended-backup-strategy)
  - [How do I completely uninstall ForgeOrchestrator?](#how-do-i-completely-uninstall-forge-orchestrator)
...
- [Config basics](#config-basics)
  ...
  - [Can the ForgeOrchestrator browser run headless?](#can-the-forge-orchestrator-browser-run-headless)
...
- [Remote gateways and nodes](#remote-gateways-and-nodes)
  ...
  - [Can two ForgeOrchestrator instances talk to each other (local + VPS)?](#can-two-forge-orchestrator-instances-talk-to-each-other-local-vps)
...
- [Env vars and .env loading](#env-vars-and-env-loading)
  - [How does ForgeOrchestrator load environment variables?](#how-does-forge-orchestrator-load-environment-variables)
...
- [Sessions and multiple chats](#sessions-and-multiple-chats)
  ...
  - [Is there a way to make a team of ForgeOrchestrator instances one CEO and many agents](#is-there-a-way-to-make-a-team-of-forge-orchestrator-instances-one-ceo-and-many-agents)
  - [Why did context get truncated mid-task? How do I prevent it?](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [How do I completely reset ForgeOrchestrator but keep it installed?](#how-do-i-completely-reset-forge-orchestrator-but-keep-it-installed)
  ...
  - [Why doesn't ForgeOrchestrator reply in a group?](#why-doesnt-forge-orchestrator-reply-in-a-group)
...
- [Models: defaults, selection, aliases, switching](#models-defaults-selection-aliases-switching)
  ...
  - [What do ForgeOrchestrator, Flawd, and Krill use for models?](#what-do-forge-orchestrator-flawd-and-krill-use-for-models)
...
- [Gateway: ports, "already running", and remote mode](#gateway-ports-already-running-and-remote-mode)
  ...
  - [Why does `forge-orchestrator gateway status` say `Runtime: running` but `RPC probe: failed`?](#why-does-forge-orchestrator-gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [Why does `forge-orchestrator gateway status` show `Config (cli)` and `Config (service)` different?](#why-does-forge-orchestrator-gateway-status-show-config-cli-and-config-service-different)
  - [What does "another gateway instance is already listening" mean?](#what-does-another-gateway-instance-is-already-listening-mean)
  - [How do I run ForgeOrchestrator in remote mode (client connects to a Gateway elsewhere)?](#how-do-i-run-forge-orchestrator-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [Logging and debugging](#logging-and-debugging)
  ...
  - [I closed my terminal on Windows - how do I restart ForgeOrchestrator?](#i-closed-my-terminal-on-windows-how-do-i-restart-forge-orchestrator)
  ...
  - [ELI5: `forge-orchestrator gateway restart` vs `forge-orchestrator gateway`](#eli5-forge-orchestrator-gateway-restart-vs-forge-orchestrator-gateway)
  - [What's the fastest way to get more details when something fails?](#whats-the-fastest-way-to-get-more-details-when-something-fails)
- [Media and attachments](#media-and-attachments)
  - [My skill generated an image/PDF, but nothing was sent](#my-skill-generated-an-imagepdf-but-nothing-was-sent)
- [Security and access control](#security-and-access-control)
  - [Is it safe to expose ForgeOrchestrator to inbound DMs?](#is-it-safe-to-expose-forge-orchestrator-to-inbound-dms)
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/forge-orchestrator/forge-orchestrator-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   forge-orchestrator health --verbose   # shows the target URL + config path on errors
   ```
...
```bash
curl -fsSL https://forge-orchestrator.ai/install.sh | bash -s -- --install-method git
```

This installs ForgeOrchestrator **from a git checkout**, so the agent can read the code + docs and
...
[https://github.com/forge-orchestrator/forge-orchestrator/issues](https://github.com/forge-orchestrator/forge-orchestrator/issues)
[https://github.com/forge-orchestrator/forge-orchestrator/pulls](https://github.com/forge-orchestrator/forge-orchestrator/pulls)
...
- `forge-orchestrator status`: quick snapshot of gateway/agent health + basic config.
- `forge-orchestrator models status`: checks provider auth + model availability.
- `forge-orchestrator doctor`: validates and repairs common config/state issues.

Other useful CLI checks: `forge-orchestrator status --all`, `forge-orchestrator logs --follow`,
`forge-orchestrator gateway status`, `forge-orchestrator health --verbose`.

Quick debug loop: [First 60 seconds if something's broken](#first-60-seconds-if-somethings-broken).
Install docs: [Install](/install), [Installer flags](/install/installer), [Updating](/install/updating).

### What's the recommended way to install and set up ForgeOrchestrator

The repo recommends running from source and using the onboarding wizard:

```bash
curl -fsSL https://forge-orchestrator.ai/install.sh | bash
forge-orchestrator onboard --install-daemon
```
...
```bash
git clone https://github.com/forge-orchestrator/forge-orchestrator.git
...
If you don't have a global install yet, run it via `pnpm forge-orchestrator onboard`.
...
- If it asks for auth, paste the token from `gateway.auth.token` (or `FORGE_ORCH_GATEWAY_TOKEN`) into Control UI settings.
- Retrieve it from the gateway host: `forge-orchestrator config get gateway.auth.token` (or generate one: `forge-orchestrator doctor --generate-gateway-token`).
...
1. Install ForgeOrchestrator on the new machine.
2. Copy `$FORGE_ORCH_STATE_DIR` (default: `~/.forge-orchestrator`) from the old machine.
3. Copy your workspace (default: `~/.forge-orchestrator/workspace`).
4. Run `forge-orchestrator doctor` and restart the Gateway service.
...
under `~/.forge-orchestrator/` (for example `~/.forge-orchestrator/agents/<agentId>/sessions/`).

Related: [Migrating](/install/migrating), [Where things live on disk](/help/faq#where-does-forge-orchestrator-store-its-data),
...
[https://github.com/forge-orchestrator/forge-orchestrator/blob/main/CHANGELOG.md](https://github.com/forge-orchestrator/forge-orchestrator/blob/main/CHANGELOG.md)
...
Some Comcast/Xfinity connections incorrectly block `docs.forge-orchestrator.ai` via Xfinity
Advanced Security. Disable it or allowlist `docs.forge-orchestrator.ai`, then retry. More
detail: [Troubleshooting](/help/troubleshooting#docsforge-orchestratorai-shows-an-ssl-error-comcastxfinity).
Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

If you still can't reach the site, the docs are mirrored on GitHub:
[https://github.com/forge-orchestrator/forge-orchestrator/tree/main/docs](https://github.com/forge-orchestrator/forge-orchestrator/tree/main/docs)
...
```bash
curl -fsSL --proto '=https' --tlsv1.2 https://forge-orchestrator.ai/install.sh | bash -s -- --beta
```

```bash
curl -fsSL --proto '=https' --tlsv1.2 https://forge-orchestrator.ai/install.sh | bash -s -- --install-method git
```

Windows installer (PowerShell):
[https://forge-orchestrator.ai/install.ps1](https://forge-orchestrator.ai/install.ps1)
...
```bash
curl -fsSL https://forge-orchestrator.ai/install.sh | bash -s -- --verbose
```

Beta install with verbose:

```bash
curl -fsSL https://forge-orchestrator.ai/install.sh | bash -s -- --beta --verbose
```

For a hackable (git) install:

```bash
curl -fsSL https://forge-orchestrator.ai/install.sh | bash -s -- --install-method git --verbose
```
...
Set-PSDebug -Trace 1
& ([scriptblock]::Create((iwr -useb https://forge-orchestrator.ai/install.ps1))) -NoOnboard
Set-PSDebug -Trace 0
```

More options: [Installer flags](/install/installer).

### Windows install says git not found or forge-orchestrator not recognized
...
**2) forge-orchestrator is not recognized after install**
...
### How do I install ForgeOrchestrator on Linux
...
### How do I install ForgeOrchestrator on a VPS
...
### Can I ask ForgeOrchestrator to update itself
...
forge-orchestrator update --channel stable|beta|dev
forge-orchestrator update --tag <dist-tag|version>
...
```bash
forge-orchestrator update --yes --no-restart
forge-orchestrator gateway restart
```
...
`forge-orchestrator onboard` is the recommended setup path. In **local mode** it walks you through:
...
No. You can run ForgeOrchestrator with **API keys** (Anthropic/OpenAI/others) or with
...
`claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in the wizard or paste it with `forge-orchestrator models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
...
Copy the token it prints, then choose **Anthropic token (paste setup-token)** in the wizard. If you want to run it on the gateway host, use `forge-orchestrator models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `forge-orchestrator models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).

### Do you support Claude subscription auth (Claude Pro or Max)

Yes - via **setup-token**. ForgeOrchestrator no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
ForgeOrchestrator supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). The wizard can run the OAuth flow and will set the default model to `openai-codex/gpt-5.3-codex` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).
...
Gemini CLI uses a **plugin auth flow**, not a client id or secret in `forge-orchestrator.json`.

Steps:

1. Enable the plugin: `forge-orchestrator plugins enable google-gemini-cli-auth`
2. Login: `forge-orchestrator models auth login --provider google-gemini-cli --set-default`
...
No. ForgeOrchestrator runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
...
### If I buy a Mac mini to run ForgeOrchestrator can I connect it to my MacBook Pro
...
- Use `forge-orchestrator nodes status` / `forge-orchestrator nodes list` to see it.
...
- DM your bot, then run `forge-orchestrator logs --follow` and read `from.id`.
...
### Can multiple people use one WhatsApp number with different ForgeOrchestrator instances
...
If you run ForgeOrchestrator via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
...
This **does not delete your data** - it only changes the ForgeOrchestrator code install. Your state
(`~/.forge-orchestrator`) and workspace (`~/.forge-orchestrator/workspace`) stay untouched.
...
```bash
npm install -g forge-orchestrator@latest
forge-orchestrator doctor
forge-orchestrator gateway restart
```
...
**ForgeOrchestrator-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

**Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

### How important is it to run ForgeOrchestrator on a dedicated machine
...
ForgeOrchestrator is lightweight. For a basic Gateway + one chat channel:
...
### Can I run ForgeOrchestrator in a VM and what are the requirements
...
## What is ForgeOrchestrator?

### What is ForgeOrchestrator in one paragraph

ForgeOrchestrator is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.

### What's the value proposition

ForgeOrchestrator is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
...
### What are the top five everyday use cases for ForgeOrchestrator
...
### Can ForgeOrchestrator help with lead gen outreach ads and blogs for a SaaS
...
ForgeOrchestrator draft and you approve.

Docs: [Security](/gateway/security).

### What are the advantages vs Claude Code for web development

ForgeOrchestrator is a **personal assistant** and coordination layer, not an IDE replacement. Use
Claude Code or Codex for the fastest direct coding loop inside a repo. Use ForgeOrchestrator when you
...
Showcase: [https://forge-orchestrator.ai/showcase](https://forge-orchestrator.ai/showcase)

## Skills and automation

### How do I customize skills without keeping the repo dirty

Use managed overrides instead of editing the repo copy. Put your changes in `~/.forge-orchestrator/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.forge-orchestrator/forge-orchestrator.json`). Precedence is `<workspace>/skills` > `~/.forge-orchestrator/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.

### Can I load skills from a custom folder

Yes. Add extra directories via `skills.load.extraDirs` in `~/.forge-orchestrator/forge-orchestrator.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.forge-orchestrator/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which ForgeOrchestrator treats as `<workspace>/skills`.
...
- Confirm cron is enabled (`cron.enabled`) and `FORGE_ORCH_SKIP_CRON` is not set.
...
```bash
forge-orchestrator cron run <jobId> --force
forge-orchestrator cron runs --id <jobId> --limit 50
```
...
### Can ForgeOrchestrator run tasks on a schedule or continuously in the background
...
Not directly. macOS skills are gated by `metadata.forge-orchestrator.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.

You have three supported patterns:

**Option A - run the Gateway on a Mac (simplest).**
Run the Gateway where the macOS binaries exist, then connect from Linux in [remote mode](#how-do-i-run-forge-orchestrator-in-remote-mode-client-connects-to-a-gateway-elsewhere) or over Tailscale. The skills load normally because the Gateway host is macOS.

**Option B - use a macOS node (no SSH).**
Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. ForgeOrchestrator can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
...
3. Override the skill metadata (workspace or `~/.forge-orchestrator/skills`) to allow Linux:

   ```markdown
   ---
   name: apple-notes
   description: Manage Apple Notes via the memo CLI on macOS.
   metadata: { "forge-orchestrator": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
   ---
   ```
...
ClawHub installs into `./skills` under your current directory (or falls back to your configured ForgeOrchestrator workspace); ForgeOrchestrator treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.forge-orchestrator/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).
...
- Persist `/home/node` with `FORGE_ORCH_HOME_VOLUME` so caches survive.
- Bake system deps into the image with `FORGE_ORCH_DOCKER_APT_PACKAGES`.
...
ForgeOrchestrator memory is just Markdown files in the agent workspace:

- Daily notes in `memory/YYYY-MM-DD.md`
- Curated long-term notes in `MEMORY.md` (main/private sessions only)

ForgeOrchestrator also runs a **silent pre-compaction memory flush** to remind the model
...
If you don't set a provider explicitly, ForgeOrchestrator auto-selects a provider when it
...
### Is all data used with ForgeOrchestrator saved locally

No - **ForgeOrchestrator's state is local**, but **external services still see what you send them**.

- **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
  (`~/.forge-orchestrator` + your workspace directory).
...
### Where does ForgeOrchestrator store its data

Everything lives under `$FORGE_ORCH_STATE_DIR` (default: `~/.forge-orchestrator`):
...
Legacy single-agent path: `~/.forge-orchestrator/agent/*` (migrated by `forge-orchestrator doctor`).

Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.forge-orchestrator/workspace`).

### Where should AGENTSmd SOULmd USERmd MEMORYmd live

These files live in the **agent workspace**, not `~/.forge-orchestrator`.

- **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
  `MEMORY.md` (or `memory.md`), `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
- **State dir (`~/.forge-orchestrator`)**: config, credentials, auth profiles, sessions, logs,
  and shared skills (`~/.forge-orchestrator/skills`).

Default workspace is `~/.forge-orchestrator/workspace`, configurable via:

```json5
{
  agents: { defaults: { workspace: "~/.forge-orchestrator/workspace" } },
}
```
...
### How do I completely uninstall ForgeOrchestrator
...
`workspace` to the repo root. The ForgeOrchestrator repo is just source code; keep the
...
ForgeOrchestrator reads an optional **JSON5** config from `$FORGE_ORCH_CONFIG_PATH` (default: `~/.forge-orchestrator/forge-orchestrator.json`):

```
$FORGE_ORCH_CONFIG_PATH
```

If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.forge-orchestrator/workspace`).

### I set gatewaybind lan or tailnet and now nothing listens the UI says unauthorized

Non-loopback binds **require auth**. Configure `gateway.auth.mode` + `gateway.auth.token` (or use `FORGE_ORCH_GATEWAY_TOKEN`).
...
key. **Recommended:** run `forge-orchestrator configure --section web` to store it in
...
- Daemons read env vars from `~/.forge-orchestrator/.env` (or the service environment).
...
### Can the ForgeOrchestrator browser run headless
...
- Gateway is running: `forge-orchestrator gateway status`
- Gateway health: `forge-orchestrator status`
- Channel health: `forge-orchestrator channels status`
...
### Can two ForgeOrchestrator instances talk to each other local VPS
...
`forge-orchestrator agent --message ... --deliver`, targeting a chat where the other bot
...
```bash
forge-orchestrator agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
```
...
- Restore from backup (git or a copied `~/.forge-orchestrator/forge-orchestrator.json`).
- If you have no backup, re-run `forge-orchestrator doctor` and reconfigure channels/models.
- If this was unexpected, file a bug and include your last known config or any backup.
- A local coding agent can often reconstruct a working config from logs or history.

Avoid it:

- Use `forge-orchestrator config set` for small changes.
- Use `forge-orchestrator configure` for interactive edits.
...
```bash
forge-orchestrator gateway --tailscale serve
```
...
### How does ForgeOrchestrator load environment variables

ForgeOrchestrator reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

- `.env` from the current working directory
- a global fallback `.env` from `~/.forge-orchestrator/.env` (aka `$FORGE_ORCH_STATE_DIR/.env`)
...
1. Put the missing keys in `~/.forge-orchestrator/.env` so they're picked up even when the service doesn't inherit your shell env.
...
`FORGE_ORCH_LOAD_SHELL_ENV=1`, `FORGE_ORCH_SHELL_ENV_TIMEOUT_MS=15000`.

### I set COPILOTGITHUBTOKEN but models status shows Shell env off Why

`forge-orchestrator models status` reports whether **shell env import** is enabled. "Shell env: off"
does **not** mean your env vars are missing - it just means ForgeOrchestrator won't load
your login shell automatically.

If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
environment. Fix by doing one of these:

1. Put the token in `~/.forge-orchestrator/.env`:
...
### Is there a way to make a team of ForgeOrchestrator instances one CEO and many agents
...
### How do I completely reset ForgeOrchestrator but keep it installed
...
```bash
forge-orchestrator reset --scope full --yes --non-interactive
```
...
- If you used profiles (`--profile` / `FORGE_ORCH_PROFILE`), reset each state dir (defaults are `~/.forge-orchestrator-<profile>`).
- Dev reset: `forge-orchestrator gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
...
headers like `# Heading`), ForgeOrchestrator skips the heartbeat run to save API calls.
...
No. ForgeOrchestrator runs on **your own account**, so if you're in the group, ForgeOrchestrator can see it.
...
```bash
forge-orchestrator logs --follow --json
```
...
```bash
forge-orchestrator directory groups list --channel whatsapp
```
...
- **Disk growth:** sessions + transcripts live under `~/.forge-orchestrator/agents/<agentId>/sessions/`.
...
- Use `forge-orchestrator doctor` to spot stray workspaces and profile mismatches.
...
ForgeOrchestrator's default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, ForgeOrchestrator currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
- `forge-orchestrator models set ...` (updates just model config)
- `forge-orchestrator configure --section model` (interactive)
- edit `agents.defaults.model` in `~/.forge-orchestrator/forge-orchestrator.json`

Avoid `config.apply` with a partial object unless you intend to replace the whole config.
If you did overwrite config, restore from backup or re-run `forge-orchestrator doctor` to repair.

Docs: [Models](/concepts/models), [Configure](/cli/configure), [Config](/cli/config), [Doctor](/gateway/doctor).

### What do ForgeOrchestrator, Flawd, and Krill use for models
...
Yes. ForgeOrchestrator ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
```
~/.forge-orchestrator/agents/<agentId>/agent/auth-profiles.json
```

Fix options:

- Run `forge-orchestrator agents add <id>` and configure auth during the wizard.
...
Cooldowns apply to failing profiles (exponential backoff), so ForgeOrchestrator can keep responding even when a provider is rate-limited or temporarily failing.
...
- **Confirm where auth profiles live** (new vs legacy paths)
  - Current: `~/.forge-orchestrator/agents/<agentId>/agent/auth-profiles.json`
  - Legacy: `~/.forge-orchestrator/agent/*` (migrated by `forge-orchestrator doctor`)
- **Confirm your env var is loaded by the Gateway**
  - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.forge-orchestrator/.env` or enable `env.shellEnv`.
- **Make sure you're editing the correct agent**
  - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
- **Sanity-check model/auth status**
  - Use `forge-orchestrator models status` to see configured models and whether providers are authenticated.
...
- **Use a setup-token**
  - Run `claude setup-token`, then paste it with `forge-orchestrator models auth setup-token --provider anthropic`.
  - If the token was created on another machine, use `forge-orchestrator models auth paste-token --provider anthropic`.
- **If you want to use an API key instead**
  - Put `ANTHROPIC_API_KEY` in `~/.forge-orchestrator/.env` on the **gateway host**.
  - Clear any pinned order that forces a missing profile:

    ```bash
    forge-orchestrator models auth order clear --provider anthropic
    ```
...
If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), ForgeOrchestrator will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
...
Fix: ForgeOrchestrator now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
ForgeOrchestrator uses provider-prefixed IDs like:
...
ForgeOrchestrator may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `forge-orchestrator models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.

You can also set a **per-agent** order override (stored in that agent's `auth-profiles.json`) via the CLI:

```bash
# Defaults to the configured default agent (omit --agent)
forge-orchestrator models auth order get --provider anthropic

# Lock rotation to a single profile (only try this one)
forge-orchestrator models auth order set --provider anthropic anthropic:default

# Or set an explicit order (fallback within provider)
forge-orchestrator models auth order set --provider anthropic anthropic:work anthropic:default
...
```bash
forge-orchestrator models auth order set --provider anthropic --agent main anthropic:default
```

### OAuth vs API key what's the difference

ForgeOrchestrator supports both:
...
```
--port > FORGE_ORCH_GATEWAY_PORT > gateway.port > default 18789
```

### Why does forge-orchestrator gateway status say Runtime running but RPC probe failed

Because "running" is the **supervisor's** view (launchd/systemd/schtasks). The RPC probe is the CLI actually connecting to the gateway WebSocket and calling `status`.

Use `forge-orchestrator gateway status` and trust these lines:

- `Probe target:` (the URL the probe actually used)
- `Listening:` (what's actually bound on the port)
- `Last gateway error:` (common root cause when the process is alive but the port isn't listening)

### Why does forge-orchestrator gateway status show Config cli and Config service different

You're editing one config file while the service is running another (often a `--profile` / `FORGE_ORCH_STATE_DIR` mismatch).
...
ForgeOrchestrator enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

Fix: stop the other instance, free the port, or run with `forge-orchestrator gateway --port <port>`.

### How do I run ForgeOrchestrator in remote mode client connects to a Gateway elsewhere
...
- `forge-orchestrator gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
...
- The Control UI stores the token in browser localStorage key `forge-orchestrator.control.settings.v1`.

Fix:

- Fastest: `forge-orchestrator dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
- If you don't have a token yet: `forge-orchestrator doctor --generate-gateway-token`.
- If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`.
- Set `gateway.auth.token` (or `FORGE_ORCH_GATEWAY_TOKEN`) on the gateway host.
- In the Control UI settings, paste the same token.
- Still stuck? Run `forge-orchestrator status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
...
- `FORGE_ORCH_CONFIG_PATH` (per-instance config)
- `FORGE_ORCH_STATE_DIR` (per-instance state)
- `agents.defaults.workspace` (workspace isolation)
- `gateway.port` (unique ports)

Quick setup (recommended):

- Use `forge-orchestrator --profile <name> …` per instance (auto-creates `~/.forge-orchestrator-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: `forge-orchestrator --profile <name> gateway install`.
...
```
forge-orchestrator tui --url ws://<host>:18789 --token <token>
```
...
```
/tmp/forge-orchestrator/forge-orchestrator-YYYY-MM-DD.log
```
...
- macOS: `$FORGE_ORCH_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.forge-orchestrator/logs/...`; profiles use `~/.forge-orchestrator-<profile>/logs/...`)
- Linux: `journalctl --user -u forge-orchestrator-gateway[-<profile>].service -n 200 --no-pager`
- Windows: `schtasks /Query /TN "ForgeOrchestrator Gateway (<profile>)" /V /FO LIST`
...
If you run the gateway manually, `forge-orchestrator gateway --force` can reclaim the port. See [Gateway](/gateway).

### I closed my terminal on Windows how do I restart ForgeOrchestrator
...
```bash
forge-orchestrator gateway run
```
...
forge-orchestrator status
forge-orchestrator models status
forge-orchestrator channels status
forge-orchestrator logs --follow
```
...
1. Is the Gateway running? `forge-orchestrator gateway status`
2. Is the Gateway healthy? `forge-orchestrator status`
3. Does the UI have the right token? `forge-orchestrator dashboard`
...
```bash
forge-orchestrator channels status
forge-orchestrator channels logs --channel telegram
```
...
### ELI5 forge-orchestrator gateway restart vs forge-orchestrator gateway

- `forge-orchestrator gateway restart`: restarts the **background service** (launchd/systemd).
- `forge-orchestrator gateway`: runs the gateway **in the foreground** for this terminal session.

If you installed the service, use the gateway commands. Use `forge-orchestrator gateway` when
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [ForgeOrchestrator assistant setup](/start/forge-orchestrator) and [Agent send](/tools/agent-send).

CLI sending:

```bash
forge-orchestrator message send --target +15555550123 --message "Here you go" --media /path/to/file.png
```
...
### Is it safe to expose ForgeOrchestrator to inbound DMs
...
Run `forge-orchestrator doctor` to surface risky DM policies.
...
```bash
forge-orchestrator pairing list telegram
```
...
No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. ForgeOrchestrator only replies to chats it receives or to explicit sends you trigger.

Approve pairing with:

```bash
forge-orchestrator pairing approve whatsapp <code>
```

List pending requests:

```bash
forge-orchestrator pairing list whatsapp
```
...
ForgeOrchestrator blocks **cross-provider** messaging by default. If a tool call is bound
