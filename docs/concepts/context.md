...
read_when:
  - You want to understand what “context” means in ForgeOrchestrator
...
“Context” is **everything ForgeOrchestrator sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (ForgeOrchestrator-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How ForgeOrchestrator builds the system prompt

The system prompt is **ForgeOrchestrator-owned** and rebuilt each run. It includes:
...
By default, ForgeOrchestrator injects a fixed set of workspace files (if present):
