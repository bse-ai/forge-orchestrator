...
Every field available in `~/.forge-orchestrator/forge-orchestrator.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — ForgeOrchestrator uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.forge-orchestrator/credentials/whatsapp/biz",
        },
      },
    },
  },
...
```json5
{
  channels: {
    discord: {
      ...
      guilds: {
        "123456789012345678": {
          slug: "friends-of-forge-orchestrator",
          requireMention: false,
...
Mattermost ships as a plugin: `forge-orchestrator plugins install @forge-orchestrator/mattermost`.
...
ForgeOrchestrator spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@forge-orchestrator", "forge-orchestrator"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@forge-orchestrator"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `forge-orchestrator.json`).
...
Default: `~/.forge-orchestrator/workspace`.
...
Optional repository root shown in the system prompt's Runtime line. If unset, ForgeOrchestrator auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/forge-orchestrator" } },
}
```
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.forge-orchestrator/sandboxes",
        docker: {
          image: "forge-orchestrator-sandbox:bookworm-slim",
          containerPrefix: "forge-orchestrator-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "forge-orchestrator-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
...
- `none`: per-scope sandbox workspace under `~/.forge-orchestrator/sandboxes`
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.forge-orchestrator/workspace",
        agentDir: "~/.forge-orchestrator/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@forge-orchestrator"] },
        sandbox: { mode: "off" },
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.forge-orchestrator/workspace-home" },
      { id: "work", workspace: "~/.forge-orchestrator/workspace-work" },
    ],
  },
  bindings: [
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.forge-orchestrator/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.forge-orchestrator/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.forge-orchestrator/workspace-public",
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.forge-orchestrator/settings/tts.json",
      elevenlabs: {
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:forge-orchestrator`   | All built-in tools (excludes provider plugins)                                           |
...
ForgeOrchestrator uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.forge-orchestrator/agents/<agentId>/agent/models.json`.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `forge-orchestrator onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `forge-orchestrator onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `forge-orchestrator onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `forge-orchestrator onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `forge-orchestrator onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `forge-orchestrator onboard --auth-choice minimax-api`.
...
- Loaded from `~/.forge-orchestrator/extensions`, `<workspace>/.forge-orchestrator/extensions`, plus `plugins.load.paths`.
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `FORGE_ORCH_GATEWAY_PORT` > `gateway.port` > `18789`.
...
```bash
FORGE_ORCH_CONFIG_PATH=~/.forge-orchestrator/a.json \
FORGE_ORCH_STATE_DIR=~/.forge-orchestrator-a \
forge-orchestrator gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.forge-orchestrator-dev` + port `19001`), `--profile <name>` (uses `~/.forge-orchestrator-<name>`).
...
Auth: `Authorization: Bearer <token>` or `x-forge-orchestrator-token: <token>`.
...
```json5
{
  hooks: {
    gmail: {
      account: "forge-orchestrator@gmail.com",
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `FORGE_ORCH_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or FORGE_ORCH_SKIP_CANVAS_HOST=1
  },
}
```
...
- Also serves A2UI at `/__forge-orchestrator__/a2ui/`.
...
- Hostname defaults to `forge-orchestrator`. Override with `FORGE_ORCH_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.forge-orchestrator/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `forge-orchestrator dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.forge-orchestrator/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${FORGE_ORCH_GATEWAY_TOKEN}" },
  },
}
```
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/forge-orchestrator/forge-orchestrator.log",
    consoleLevel: "info",
...
- Default log file: `/tmp/forge-orchestrator/forge-orchestrator-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `forge-orchestrator doctor --fix` can strip unknown keys).
...
```json5
// ~/.forge-orchestrator/forge-orchestrator.json
{
  gateway: { port: 18789 },
