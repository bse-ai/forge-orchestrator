...
If you run `--deep`, ForgeOrchestrator also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.forge-orchestrator/credentials/whatsapp/<accountId>/creds.json`
...
`forge-orchestrator security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${FORGE_ORCH_GATEWAY_PASSWORD}
```
...
ForgeOrchestrator stores session transcripts on disk under `~/.forge-orchestrator/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.forge-orchestrator` (see the audit section below). If you need
...
ForgeOrchestrator can refresh the skills list mid-session:
...
- If you install plugins from npm (`forge-orchestrator plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.forge-orchestrator/extensions/<pluginId>/` (or `$FORGE_ORCH_STATE_DIR/extensions/<pluginId>/`).
  - ForgeOrchestrator uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
forge-orchestrator pairing approve <channel> <code>
```
...
By default, ForgeOrchestrator routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
ForgeOrchestrator has two separate “who can trigger me?” layers:
...
- “Paste the full contents of ~/.forge-orchestrator or your logs.”
...
- `~/.forge-orchestrator/forge-orchestrator.json`: `600` (user read/write only)
- `~/.forge-orchestrator`: `700` (user only)

`forge-orchestrator doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `FORGE_ORCH_GATEWAY_PORT`
...
The Gateway broadcasts its presence via mDNS (`_forge-orchestrator-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `FORGE_ORCH_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `forge-orchestrator doctor --generate-gateway-token`.
...
1. Generate/set a new secret (`gateway.auth.token` or `FORGE_ORCH_GATEWAY_PASSWORD`).
...
- ForgeOrchestrator will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.forge-orchestrator/` (or `$FORGE_ORCH_STATE_DIR/`) may contain secrets or private data:

- `forge-orchestrator.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `forge-orchestrator status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@forge-orchestrator", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.forge-orchestrator/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `forge-orchestrator` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only ForgeOrchestrator clients can connect.
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `forge-orchestrator gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `FORGE_ORCH_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/forge-orchestrator/forge-orchestrator-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.forge-orchestrator/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `openclaw security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + ForgeOrchestrator version
...
Found a vulnerability in ForgeOrchestrator? Please report responsibly:

1. Email: [security@forge-orchestrator.ai](mailto:security@forge-orchestrator.ai)
