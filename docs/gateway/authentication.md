...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.forge-orchestrator/.env` so the daemon can read it:
...
API keys for daemon use: `forge-orchestrator onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.forge-orchestrator/.env`, systemd/launchd).
...
Then paste it into ForgeOrchestrator:
...
```bash
forge-orchestrator models auth paste-token --provider anthropic
forge-orchestrator models auth paste-token --provider openrouter
```
...
```bash
forge-orchestrator models status --check
```
...
Run `forge-orchestrator models status` to confirm which profile is expiring. If the profile
