...
ForgeOrchestrator’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`forge-orchestrator agent`), so routing/permissions/config match your Gateway.
