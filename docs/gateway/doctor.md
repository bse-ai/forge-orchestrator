...
`forge-orchestrator doctor` is the repair + migration tool for ForgeOrchestrator. It fixes stale
...
```bash
forge-orchestrator doctor --yes
```

Accept defaults without prompting (including restart/service/sandbox repair steps when applicable).

```bash
forge-orchestrator doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
forge-orchestrator doctor --repair --force
```

Apply aggressive repairs too (overwrites custom supervisor configs).

```bash
forge-orchestrator doctor --non-interactive
```
...
```bash
forge-orchestrator doctor --deep
```
...
```bash
cat ~/.forge-orchestrator/forge-orchestrator.json
```
...
- Extra workspace dir detection (`~/forge-orchestrator`).
...
you to run `forge-orchestrator doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.forge-orchestrator/forge-orchestrator.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.forge-orchestrator/sessions/` to `~/.forge-orchestrator/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.forge-orchestrator/agent/` to `~/.forge-orchestrator/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.forge-orchestrator/credentials/*.json` (except `oauth.json`)
  - to `~/.forge-orchestrator/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `forge-orchestrator doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.forge-orchestrator` folders exist across
  home directories or when `FORGE_ORCH_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.forge-orchestrator/forge-orchestrator.json` is
...
offers to remove them and install the ForgeOrchestrator service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named ForgeOrchestrator gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `forge-orchestrator doctor --generate-gateway-token` to force token
...
- `forge-orchestrator doctor` prompts before rewriting supervisor config.
- `forge-orchestrator doctor --yes` accepts the default repair prompts.
- `forge-orchestrator doctor --repair` applies recommended fixes without prompts.
- `forge-orchestrator doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `forge-orchestrator gateway install --force`.
