...
- During heartbeat runs, ForgeOrchestrator treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `forge-orchestrator sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, ForgeOrchestrator skips the heartbeat run entirely (no model call).
...
```bash
forge-orchestrator system event --text "Check for urgent follow-ups" --mode now
```
