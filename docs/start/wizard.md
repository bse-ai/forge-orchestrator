...
The onboarding wizard is the **recommended** way to set up ForgeOrchestrator on macOS,
...
`forge-orchestrator dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
forge-orchestrator agents add <name>
```
...
(`web_fetch` works without a key). Easiest path: `forge-orchestrator configure --section web`
...
Use `forge-orchestrator agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.forge-orchestrator/workspace-<agentId>`.
...
- CLI command reference: [`forge-orchestrator onboard`](/cli/onboard)
