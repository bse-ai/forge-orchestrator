...
read_when:
  - You need detailed behavior for forge-orchestrator onboard
...
This page is the full reference for `forge-orchestrator onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.forge-orchestrator/forge-orchestrator.json` exists, choose Keep, Modify, or Reset.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `forge-orchestrator pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `forge-orchestrator health`.
    - `forge-orchestrator status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.forge-orchestrator/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.forge-orchestrator/agents/<agentId>/agent/auth-profiles.json`
...
`~/.forge-orchestrator/credentials/oauth.json` (or `$FORGE_ORCH_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`forge-orchestrator onboard`](/cli/onboard)
