---
summary: "ForgeOrchestrator CLI reference for `forge-orchestrator` commands, subcommands, and options"
read_when:
...
- `--dev`: isolate state under `~/.forge-orchestrator-dev` and shift default ports.
- `--profile <name>`: isolate state under `~/.forge-orchestrator-<name>`.
- `--no-color`: disable ANSI colors.
- `--update`: shorthand for `forge-orchestrator update` (source installs only).
...
ForgeOrchestrator uses a lobster palette for CLI output.
...
```
forge-orchestrator [--dev] [--profile <name>] <command>
  setup
  onboard
  configure
  config
...
Note: plugins can add additional top-level commands (for example `forge-orchestrator voicecall`).

## Security

- `forge-orchestrator security audit` — audit config + local state for common security foot-guns.
- `forge-orchestrator security audit --deep` — best-effort live Gateway probe.
- `forge-orchestrator security audit --fix` — tighten safe defaults and chmod state/config.
...
- `forge-orchestrator plugins list` — discover plugins (use `--json` for machine output).
- `forge-orchestrator plugins info <id>` — show details for a plugin.
- `forge-orchestrator plugins install <path|.tgz|npm-spec>` — install a plugin (or add a plugin path to `plugins.load.paths`).
- `forge-orchestrator plugins enable <id>` / `disable <id>` — toggle `plugins.entries.<id>.enabled`.
- `forge-orchestrator plugins doctor` — report plugin load errors.
...
- `--workspace <dir>`: agent workspace path (default `~/.forge-orchestrator/workspace`).
...
- `channels status`: check gateway reachability and channel health (`--probe` runs extra checks; use `forge-orchestrator health` or `forge-orchestrator status --deep` for gateway health probes).
- Tip: `channels status` prints warnings with suggested fixes when it can detect common misconfigurations (then points you to `forge-orchestrator doctor`).
...
```bash
forge-orchestrator channels add --channel telegram --account alerts --name "Alerts Bot" --token $TELEGRAM_BOT_TOKEN
forge-orchestrator channels add --channel discord --account work --name "Work Bot" --token $DISCORD_BOT_TOKEN
forge-orchestrator channels remove --channel discord --account work --delete
...
- `forge-orchestrator message send --target +15555550123 --message "Hi"`
- `forge-orchestrator message poll --channel discord --target channel:123 --poll-question "Snack?" --poll-option Pizza --poll-option Sushi`
...
ForgeOrchestrator can surface provider usage/quota when OAuth/API creds are available.

Surfaces:

- `/status` (adds a short provider usage line when available)
- `forge-orchestrator status --usage` (prints full provider breakdown)
...
- `gateway status` also surfaces legacy or extra gateway services when it can detect them (`--deep` adds system-level scans). Profile-named ForgeOrchestrator services are treated as first-class and aren't flagged as "extra".
...
forge-orchestrator logs --limit 200
forge-orchestrator logs --plain
forge-orchestrator logs --json
forge-orchestrator logs --no-color
```
...
```bash
claude setup-token
forge-orchestrator models auth setup-token --provider anthropic
...
`forge-orchestrator models` is an alias for `models status`.
...
[`forge-orchestrator node`](/cli/node).
...
Browser control CLI (dedicated Chrome/Brave/Edge/Chromium). See [`forge-orchestrator browser`](/cli/browser) and the [Browser tool](/tools/browser).
