...
```bash
forge-orchestrator acp

# Remote Gateway
forge-orchestrator acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
openclaw acp --url wss://gateway-host:18789 --token-file ~/.openclaw/gateway.token

# Attach to an existing session key
forge-orchestrator acp --session agent:main:main

# Attach by label (must already exist)
forge-orchestrator acp --session-label "support inbox"

# Reset the session key before the first prompt
forge-orchestrator acp --session agent:main:main --reset-session
```
...
```bash
forge-orchestrator acp client

# Point the spawned bridge at a remote Gateway
openclaw acp client --server-args --url wss://gateway-host:18789 --token-file ~/.openclaw/gateway.token

# Override the server command (default: forge-orchestrator)
forge-orchestrator acp client --server "node" --server-args forge-orchestrator.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive a ForgeOrchestrator Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `forge-orchestrator acp` over stdio.

Example config (persisted):

```bash
forge-orchestrator config set gateway.remote.url wss://gateway-host:18789
forge-orchestrator config set gateway.remote.token <token>
```
...
```bash
forge-orchestrator acp --session agent:main:main
forge-orchestrator acp --session agent:design:main
forge-orchestrator acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “ForgeOrchestrator ACP” to start a thread.
...
- `--server <command>`: ACP server command (default: `forge-orchestrator`).
