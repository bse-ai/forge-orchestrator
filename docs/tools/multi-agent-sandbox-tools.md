...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `forge-orchestrator sandbox explain`.
...
Legacy `agent.*` configs are migrated by `forge-orchestrator doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=forge-orchestrator-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${FORGE_ORCH_STATE_DIR:-$HOME/.forge-orchestrator}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
