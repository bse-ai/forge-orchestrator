...
ForgeOrchestrator ships two lightweight web tools:
...
3. Run `forge-orchestrator configure --section web` to store the key in config (recommended), or set `BRAVE_API_KEY` in your environment.
...
**Recommended:** run `forge-orchestrator configure --section web`. It stores the key in
`~/.forge-orchestrator/forge-orchestrator.json` under `tools.web.search.apiKey`.

**Environment alternative:** set `BRAVE_API_KEY` in the Gateway process
environment. For a gateway install, put it in `~/.forge-orchestrator/.env` (or your
service environment). See [Env vars](/help/faq#how-does-forge-orchestrator-load-environment-variables).
...
environment. For a gateway install, put it in `~/.forge-orchestrator/.env`.

If no base URL is set, ForgeOrchestrator chooses a default based on the API key source:
