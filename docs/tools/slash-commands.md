...
- `commands.config` (default `false`) enables `/config` (reads/writes `forge-orchestrator.json`).
...
- For full provider usage breakdown, use `forge-orchestrator status --usage`.
...
- Overrides apply immediately to new config reads, but do **not** write to `forge-orchestrator.json`.
- Use `/debug reset` to clear all overrides and return to the on-disk config.

## Config updates

`/config` writes to your on-disk config (`forge-orchestrator.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
