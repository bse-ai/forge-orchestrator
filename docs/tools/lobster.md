---
title: Lobster
summary: "Typed workflow runtime for ForgeOrchestrator with resumable approval gates."
description: Typed workflow runtime for ForgeOrchestrator — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets ForgeOrchestrator run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: ForgeOrchestrator runs one Lobster tool call and gets a structured result.
...
ForgeOrchestrator launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | forge-orchestrator.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
forge-orchestrator.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In ForgeOrchestrator tool calls, set `pipeline` to the file path.
...
plugin tools (like `lobster`), ForgeOrchestrator keeps core tools enabled. To restrict core
...
→ forge-orchestrator calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ forge-orchestrator calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls ForgeOrchestrator tools that do.
