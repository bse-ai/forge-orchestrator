...
- **gateway host** → `forge-orchestrator` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.forge-orchestrator/exec-approvals.sock",
...
`~/.forge-orchestrator/exec-approvals.json` directly.

CLI: `forge-orchestrator approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
