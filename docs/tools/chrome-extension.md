---
summary: "Chrome extension: let ForgeOrchestrator drive your existing Chrome tab"
read_when:
...
The ForgeOrchestrator Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate forge-orchestrator-managed Chrome profile.
...
ForgeOrchestrator then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the ForgeOrchestrator release (npm package) as static files. There is no separate “build” step.

After upgrading ForgeOrchestrator:

- Re-run `forge-orchestrator browser extension install` to refresh the installed files under your ForgeOrchestrator state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

ForgeOrchestrator ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want ForgeOrchestrator to control.
...
- `ON`: attached; ForgeOrchestrator can drive that tab.
...
Debugging: `forge-orchestrator sandbox explain`
...
`forge-orchestrator browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `forge-orchestrator browser extension install` first to copy the extension to a stable location under your ForgeOrchestrator state directory.
...
- **This is not isolated** like the dedicated forge-orchestrator-managed profile.
