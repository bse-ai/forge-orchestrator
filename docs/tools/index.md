---
summary: "Agent tool surface for ForgeOrchestrator (browser, canvas, nodes, message, cron) replacing legacy `forge-orchestrator-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `forge-orchestrator-*` skills
title: "Tools"
---

# Tools (ForgeOrchestrator)

ForgeOrchestrator exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `forge-orchestrator-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `forge-orchestrator.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, ForgeOrchestrator logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:forge-orchestrator`: all built-in ForgeOrchestrator tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`forge-orchestrator node run`).
...
- Requires a Brave API key (recommended: `forge-orchestrator configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated ForgeOrchestrator-managed browser.
...
- Quick smoke: `forge-orchestrator nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `forge-orchestrator gateway` restart in-place)
