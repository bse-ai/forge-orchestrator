...
async function makeTempDir(label: string): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), `forge-orchestrator-${label}-`));
}
...
function buildParams(params: { config?: ForgeOrchestratorConfig; workspaceDir?: string; cwd?: string }) {
...
describe("buildSystemPromptParams repo root", () => {
  ...
  it("uses configured repoRoot when valid", async () => {
    ...
    const config: ForgeOrchestratorConfig = {
      agents: {
        defaults: {
          repoRoot,
        },
