...
describe("createOpenClawCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createForgeOrchestratorCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "forge-orchestrator-outside.txt");
