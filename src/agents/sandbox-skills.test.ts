...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "forge-orchestrator-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-bundled-skills-"));
    await fs.mkdir(bundledDir, { recursive: true });

    process.env.FORGE_ORCH_BUNDLED_SKILLS_DIR = bundledDir;

    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-workspace-"));
    await writeSkill({
