...
describe("resolveBundledSkillsDir", () => {
  ...
  it("resolves bundled skills under a flattened dist layout", async () => {
    delete process.env.FORGE_ORCH_BUNDLED_SKILLS_DIR;

    const root = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "forge-orchestrator" }));
