...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    note.mockClear();
    pluginRegistry.list = [];
    prevToken = process.env.FORGE_ORCH_GATEWAY_TOKEN;
    prevPassword = process.env.FORGE_ORCH_GATEWAY_PASSWORD;
  ...
  afterEach(() => {
    if (prevToken === undefined) {
      ...
      process.env.FORGE_ORCH_GATEWAY_TOKEN = prevToken;
    }
    if (prevPassword === undefined) {
      delete process.env.FORGE_ORCH_GATEWAY_PASSWORD;
    } else {
      process.env.FORGE_ORCH_GATEWAY_PASSWORD = prevPassword;
    }
  });
  ...
  it("warns when exposed without auth", async () => {
    const cfg = { gateway: { bind: "lan" } } as ForgeOrchestratorConfig;
  ...
  it("uses env token to avoid critical warning", async () => {
    process.env.FORGE_ORCH_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as ForgeOrchestratorConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("skips warning for loopback bind", async () => {
    const cfg = { gateway: { bind: "loopback" } } as ForgeOrchestratorConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("shows explicit dmScope config command for multi-user DMs", async () => {
    ...
    const cfg = { session: { dmScope: "main" } } as ForgeOrchestratorConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
