...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "FORGE_ORCH_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("FORGE_ORCH_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv FORGE_ORCH_GATEWAY_TOKEN");
    expect(message).not.toContain("FORGE_ORCH_GATEWAY_PASSWORD");
  });

  it("does nothing when config has no gateway credentials", async () => {
    const noteFn = vi.fn();
    const getenv = vi.fn(async () => "launchctl-token");
    const cfg = {} as ForgeOrchestratorConfig;
