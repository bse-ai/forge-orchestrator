...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("forge-orchestrator models auth setup-token")}\` or \`${formatCliCommand("forge-orchestrator configure")}\`.`
          : `Run \`${formatCliCommand("forge-orchestrator configure")}\` or set an API key env var.`;
