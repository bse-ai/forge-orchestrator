...
export function resolveTargets(cfg: ForgeOrchestratorConfig, explicitUrl?: string): GatewayStatusTarget[] {
...
export function resolveAuthForTarget(
  ...
  const envToken = process.env.FORGE_ORCH_GATEWAY_TOKEN?.trim() || "";
  const envPassword = process.env.FORGE_ORCH_GATEWAY_PASSWORD?.trim() || "";
  const cfgToken =
...
export function buildNetworkHints(cfg: ForgeOrchestratorConfig) {
