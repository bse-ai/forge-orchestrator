...
vi.mock("../../plugins/loader.js", () => ({
  loadForgeOrchestratorPlugins: vi.fn(),
}));
...
const baseEntry: ChannelPluginCatalogEntry = {
  ...
  install: {
    npmSpec: "@forge-orchestrator/zalo",
...
describe("ensureOnboardingPluginInstalled", () => {
  it("installs from npm and enables the plugin", async () => {
    ...
    const cfg: ForgeOrchestratorConfig = { plugins: { allow: ["other"] } };
    ...
    expect(result.cfg.plugins?.installs?.zalo?.spec).toBe("@forge-orchestrator/zalo");
    expect(result.cfg.plugins?.installs?.zalo?.installPath).toBe("/tmp/zalo");
    expect(installPluginFromNpmSpec).toHaveBeenCalledWith(
      expect.objectContaining({ spec: "@forge-orchestrator/zalo" }),
    );
  });
