...
describe("doctor command", () => {
  ...
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "forge-orchestrator-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.FORGE_ORCH_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when OPENCLAW_GATEWAY_TOKEN is set", async () => {
    ...
    const prevToken = process.env.FORGE_ORCH_GATEWAY_TOKEN;
    process.env.FORGE_ORCH_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
