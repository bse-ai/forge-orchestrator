...
const SEARCH_TOOL = "https://docs.forge-orchestrator.ai/mcp.SearchForgeOrchestrator";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.forge-orchestrator.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('forge-orchestrator docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.forge-orchestrator.ai/");
      runtime.log(`Search: ${formatCliCommand('forge-orchestrator docs "your query"')}`);
    }
    return;
  }
