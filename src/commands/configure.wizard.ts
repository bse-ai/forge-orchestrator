    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from forge-orchestrator.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "ForgeOrchestrator update wizard" : "ForgeOrchestrator configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: ForgeOrchestratorConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.forge-orchestrator.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("forge-orchestrator doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.FORGE_ORCH_GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env.FORGE_ORCH_GATEWAY_PASSWORD,
    });
    ...
    let gatewayToken: string | undefined =
      ...
      process.env.FORGE_ORCH_GATEWAY_TOKEN;
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.FORGE_ORCH_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.FORGE_ORCH_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.FORGE_ORCH_GATEWAY_TOKEN;
