...
beforeEach(() => {
  ...
  setStdinTty(true);
  originalStateDir = process.env.FORGE_ORCH_STATE_DIR;
  originalUpdateInProgress = process.env.FORGE_ORCH_UPDATE_IN_PROGRESS;
  process.env.FORGE_ORCH_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "forge-orchestrator-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.FORGE_ORCH_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.FORGE_ORCH_UPDATE_IN_PROGRESS;
  } else {
    process.env.FORGE_ORCH_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
