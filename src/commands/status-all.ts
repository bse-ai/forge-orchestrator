...
import { resolveForgeOrchestratorPackageRoot } from "../infra/forge-orchestrator-root.js";
...
export async function statusAllCommand(
  ...
  await withProgress({ label: "Scanning status --all…", total: 11 }, async (progress) => {
    ...
    const overviewRows = [
      ...
      { Item: "Security", Value: `Run: ${formatCliCommand("forge-orchestrator security audit --deep")}` },
      gatewaySelfLine
