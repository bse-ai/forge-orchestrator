...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "forge-orchestrator" | "forge-orchestrator" | "forge-orchestrator";
  legacy?: boolean;
};
...
export function renderGatewayServiceCleanupHints(
  ...
  const profile = env.FORGE_ORCH_PROFILE;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["forge-orchestrator_service_marker"];
  const kindKeys = ["forge-orchestrator_service_kind"];
...
function isForgeOrchestratorGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.forge-orchestrator.");
}

function isForgeOrchestratorGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("forge-orchestrator-gateway")) {
    return false;
  }
  return contents.toLowerCase().includes("gateway");
}

function isForgeOrchestratorGatewayTaskName(name: string): boolean {
  const normalized = name.trim().toLowerCase();
  if (!normalized) {
    return false;
  }
  const defaultName = resolveGatewayWindowsTaskName().toLowerCase();
  return normalized === defaultName || normalized.startsWith("forge-orchestrator gateway");
}
...
function isLegacyLabel(label: string): boolean {
  ...
  return lower.includes("forge-orchestrator") || lower.includes("forge-orchestrator");
}
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    if (!marker) {
      ...
      results.push({
        ...
        scope: params.scope,
        marker: isLegacyLabel(label) ? "forge-orchestrator" : "forge-orchestrator",
        legacy: true,
      });
      continue;
    }
    ...
    if (marker === "forge-orchestrator" && isForgeOrchestratorGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "forge-orchestrator" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (marker === "forge-orchestrator" && isForgeOrchestratorGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "forge-orchestrator",
    });
  }
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isForgeOrchestratorGatewayTaskName(name)) {
        continue;
      }
