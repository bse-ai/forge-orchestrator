...
describe("resolveConfigDir", () => {
  it("prefers ~/.forge-orchestrator when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers FORGE_ORCH_HOME over HOME", () => {
    vi.stubEnv("FORGE_ORCH_HOME", "/srv/forge-orchestrator-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/forge-orchestrator-home"));
...
describe("shortenHomePath", () => {
  it("uses $FORGE_ORCH_HOME prefix when FORGE_ORCH_HOME is set", () => {
    vi.stubEnv("FORGE_ORCH_HOME", "/srv/forge-orchestrator-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/forge-orchestrator-home")}/.forge-orchestrator/forge-orchestrator.json`)).toBe(
      "$FORGE_ORCH_HOME/.forge-orchestrator/forge-orchestrator.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $FORGE_ORCH_HOME replacement when FORGE_ORCH_HOME is set", () => {
    vi.stubEnv("FORGE_ORCH_HOME", "/srv/forge-orchestrator-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/forge-orchestrator-home")}/.forge-orchestrator/forge-orchestrator.json`),
    ).toBe("config: $FORGE_ORCH_HOME/.forge-orchestrator/forge-orchestrator.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/forge-orchestrator")).toBe(path.resolve(os.homedir(), "forge-orchestrator"));
  });
  ...
  it("prefers FORGE_ORCH_HOME for tilde expansion", () => {
    vi.stubEnv("FORGE_ORCH_HOME", "/srv/forge-orchestrator-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/forge-orchestrator")).toBe(path.resolve("/srv/forge-orchestrator-home", "forge-orchestrator"));
