...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    const started = await startGatewayBonjourAdvertiser({
      ...
      cliPath: "/opt/homebrew/bin/forge-orchestrator",
    });

    expect(createService).toHaveBeenCalledTimes(1);
    const [gatewayCall] = createService.mock.calls as Array<[Record<string, unknown>]>;
    expect(gatewayCall?.[0]?.type).toBe("forge-orch-gw");
    ...
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.cliPath).toBe(
      "/opt/homebrew/bin/forge-orchestrator",
    );
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("forge-orchestrator (ForgeOrchestrator)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("forge-orchestrator");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("forge-orchestrator.local");
