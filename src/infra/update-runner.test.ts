...
describe("runGatewayUpdate", () => {
  ...
  it("skips update when no git root", async () => {
    await fs.writeFile(
      ...
      JSON.stringify({ name: "forge-orchestrator", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
  ...
  async function runNpmGlobalUpdateCase(params: {
    ...
    const { calls, runCommand } = createGlobalInstallHarness({
      ...
      onInstall: async () => {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
          JSON.stringify({ name: "forge-orchestrator", version: "2.0.0" }),
          "utf-8",
        );
      },
    });
  ...
  it("cleans stale npm rename dirs before global update", async () => {
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "forge-orchestrator");
    const staleDir = path.join(nodeModules, ".forge-orchestrator-stale");
  ...
  it("rejects git roots that are not a forge-orchestrator checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    ...
    expect(result.status).toBe("error");
    expect(result.reason).toBe("not-forge-orchestrator-root");
  ...
  it("fails with a clear reason when openclaw.mjs is missing", async () => {
    ...
    expect(result.steps.at(-1)?.name).toBe("forge-orchestrator doctor entry");
  });
