...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of Forge Orchestrator")).toBe("friends-of-forge-orchestrator");
  ...
  it("matches ids by default and names only when enabled", () => {
    const allow = normalizeDiscordAllowList(
      ["123", "steipete", "Friends of Forge Orchestrator"],
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-forge-orchestrator" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of Forge Orchestrator"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-forge-orchestrator");
  });

  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-forge-orchestrator": { slug: "friends-of-forge-orchestrator" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of Forge Orchestrator"),
      guildEntries,
...
describe("discord group DM gating", () => {
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["forge-orchestrator-dm"],
        channelId: "1",
        channelName: "ForgeOrchestrator DM",
        channelSlug: "forge-orchestrator-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
