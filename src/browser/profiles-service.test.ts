...
vi.mock("./chrome.js", () => ({
  resolveForgeOrchestratorUserDataDir: vi.fn(() => "/tmp/forge-orchestrator-test/forge-orchestrator/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveForgeOrchestratorUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "forge-orchestrator-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveForgeOrchestratorUserDataDir).mockReturnValue(userDataDir);
