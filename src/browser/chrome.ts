...
import {
  decorateForgeOrchestratorProfile,
...
export function resolveForgeOrchestratorUserDataDir(profileName = DEFAULT_FORGE_ORCH_BROWSER_PROFILE_NAME) {
...
export async function launchForgeOrchestratorChrome(
  ...
  const userDataDir = resolveForgeOrchestratorUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_FORGE_ORCH_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateForgeOrchestratorProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 forge-orchestrator browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`forge-orchestrator browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`forge-orchestrator browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 forge-orchestrator browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
