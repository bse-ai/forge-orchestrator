import type { BrowserConfig, BrowserProfileConfig, ForgeOrchestratorConfig } from "../config/config.js";
...
import {
  DEFAULT_FORGE_ORCH_BROWSER_COLOR,
  DEFAULT_FORGE_ORCH_BROWSER_ENABLED,
  DEFAULT_BROWSER_EVALUATE_ENABLED,
  DEFAULT_BROWSER_DEFAULT_PROFILE_NAME,
  DEFAULT_FORGE_ORCH_BROWSER_PROFILE_NAME,
} from "./constants.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_FORGE_ORCH_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "forge-orchestrator" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_FORGE_ORCH_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_FORGE_ORCH_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an ForgeOrchestrator browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for forge-orchestrator/forge-orchestrator CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: ForgeOrchestratorConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_FORGE_ORCH_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "forge-orchestrator";
