...
<title>ForgeOrchestrator Canvas</title>
<style>
...
<div class="wrap">
  <div class="card">
    <div class="title">
      <h1>ForgeOrchestrator Canvas</h1>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.forgeOrchCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.forgeOrchCanvasA2UIAction &&
        typeof window.forgeOrchCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.forgeOrchSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("forge-orchestrator:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android ForgeOrchestrator node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.forgeOrchSendUserAction === "function"
        ? window.forgeOrchSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.FORGE_ORCH_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.FORGE_ORCH_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>ForgeOrchestrator Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
