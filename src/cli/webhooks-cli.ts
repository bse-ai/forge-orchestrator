...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.forge-orchestrator.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + ForgeOrchestrator hooks")
    ...
    .option("--hook-url <url>", "ForgeOrchestrator hook URL")
    .option("--hook-token <token>", "ForgeOrchestrator hook token")
