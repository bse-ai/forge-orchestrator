...
function applySlotSelectionForPlugin(
  ...
  pluginId: string,
): { config: ForgeOrchestratorConfig; warnings: string[] } {
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.forge-orchestrator.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.forge-orchestrator.ai/plugin");
      lines.push("");
