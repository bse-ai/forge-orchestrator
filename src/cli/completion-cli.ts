...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# ForgeOrchestrator Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(forge-orchestrator completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# ForgeOrchestrator Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(forge-orchestrator completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option(
      "--write-state",
      "Write completion scripts to $FORGE_ORCH_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "forge-orchestrator") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c forge-orchestrator -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'forge-orchestrator gateway' and not 'forge-orchestrator other gateway', we need to check the sequence.
