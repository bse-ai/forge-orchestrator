...
import { ensureForgeOrchestratorModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.FORGE_ORCH_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.FORGE_ORCH_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.FORGE_ORCH_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  cfg: ForgeOrchestratorConfig;
  agentDir: string;
}): ForgeOrchestratorConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<ForgeOrchestratorConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.FORGE_ORCH_AGENT_DIR,
  ...
  const hostAgentDir = resolveForgeOrchestratorAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-live-state-"));
  ...
  process.env.FORGE_ORCH_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.forge-orchestrator-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveForgeOrchestratorAgentDir();
  const sanitizedCfg: ForgeOrchestratorConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-live-"));
  ...
  await ensureForgeOrchestratorModelsJson(nextCfg);
  ...
  try {
          ...
          if (params.extraToolProbes) {
            ...
            const execReadProbe = await client.request<AgentFinalPayload>(
              "agent",
              {
                ...
                message:
                  "ForgeOrchestrator live tool probe (local, safe): " +
    ...
    process.env.FORGE_ORCH_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.FORGE_ORCH_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureForgeOrchestratorModelsJson(cfg);
      ...
      const rawModels = process.env.FORGE_ORCH_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.forge-orchestrator-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
