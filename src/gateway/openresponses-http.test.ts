...
async function writeGatewayConfig(config: Record<string, unknown>) {
  const configPath = process.env.FORGE_ORCH_CONFIG_PATH;
  if (!configPath) {
    throw new Error("FORGE_ORCH_CONFIG_PATH is required for gateway config tests");
  }
  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "forge-orchestrator", input: "hi" }),
      });
      ...
      const resHeader = await postResponses(
        port,
        { model: "forge-orchestrator", input: "hi" },
        { "x-forge-orchestrator-agent-id": "beta" },
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "forge-orchestrator:beta", input: "hi" });
