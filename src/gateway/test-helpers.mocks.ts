...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `forge-orchestrator-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.FORGE_ORCH_CONFIG_PATH = path.join(root, "forge-orchestrator.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "forge-orchestrator.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "forge-orchestrator-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadForgeOrchestratorPlugins: () => pluginRegistryState.registry,
  };
});
