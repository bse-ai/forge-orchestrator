...
import { ensureForgeOrchestratorCliOnPath } from "../infra/path-env.js";
...
ensureForgeOrchestratorCliOnPath();
...
export async function startGatewayServer(
  ...
  process.env.FORGE_ORCH_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "FORGE_ORCH_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "FORGE_ORCH_RAW_STREAM_PATH",
  ...
  if (configSnapshot.legacyIssues.length > 0) {
    ...
    if (!migrated) {
      throw new Error(
        `Legacy config entries detected but auto-migration failed. Run "${formatCliCommand("forge-orchestrator doctor")}" to migrate.`,
      );
    }
  ...
  if (configSnapshot.exists && !configSnapshot.valid) {
    ...
    throw new Error(
      `Invalid config at ${configSnapshot.path}.\n${issues}\nRun "${formatCliCommand("forge-orchestrator doctor")}" to repair, then retry.`,
    );
  }
