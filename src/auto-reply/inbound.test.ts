...
describe("initSessionState BodyStripped", () => {
  it("prefers BodyForAgent over Body for group chats", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-sender-meta-"));
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath } } as ForgeOrchestratorConfig;
  ...
  it("prefers BodyForAgent over Body for direct chats", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-sender-meta-direct-"));
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath } } as ForgeOrchestratorConfig;
...
describe("mention helpers", () => {
  it("builds regexes and skips invalid patterns", () => {
    const regexes = buildMentionRegexes({
      messages: {
        groupChat: { mentionPatterns: ["\\bforge-orchestrator\\b", "(invalid"] },
      },
    });
    expect(regexes).toHaveLength(1);
    expect(regexes[0]?.test("forge-orchestrator")).toBe(true);
  });

  it("normalizes zero-width characters", () => {
    expect(normalizeMentionText("forge\u200b-orchestrator")).toBe("forge-orchestrator");
  });

  it("matches patterns case-insensitively", () => {
    const regexes = buildMentionRegexes({
      messages: { groupChat: { mentionPatterns: ["\\bforge-orchestrator\\b"] } },
    });
    expect(matchesMentionPatterns("FORGE-ORCHESTRATOR: hi", regexes)).toBe(true);
  });
