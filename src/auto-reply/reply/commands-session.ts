...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting ForgeOrchestrator in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerForgeOrchestratorRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting ForgeOrchestrator via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
