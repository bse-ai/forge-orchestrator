...
import {
  parseFrontmatter,
  resolveForgeOrchestratorMetadata,
...
`;
    ...
    expect(parsed["forge-orchestrator"].emoji).toBe("💾");
    expect(parsed["forge-orchestrator"].events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed["forge-orchestrator"].emoji).toBe("📝");
    expect(parsed["forge-orchestrator"].events).toEqual(["command"]);
    expect(parsed["forge-orchestrator"].requires.config).toEqual(["workspace.dir"]);
    expect(parsed["forge-orchestrator"].install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"forge-orchestrator": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"forge-orchestrator": {"events": ["test"]}}');
  });
...
describe("resolveForgeOrchestratorMetadata", () => {
  it("extracts forge-orchestrator metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveForgeOrchestratorMetadata(frontmatter);
  ...
  it("returns undefined when forge-orchestrator key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        "forge-orchestrator": {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with ForgeOrchestrator" },
            { id: "npm", kind: "npm", package: "@forge-orchestrator/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@forge-orchestrator/hook");
  });
...
`;
    ...
    expect(forgeOrchMeta?.emoji).toBe("disk");
