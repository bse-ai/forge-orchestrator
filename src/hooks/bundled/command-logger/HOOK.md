...
2. **Appends to log file** - Writes a JSON line to `~/.forge-orchestrator/logs/commands.log`
...
`~/.forge-orchestrator/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.forge-orchestrator/logs/commands.log ~/.forge-orchestrator/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/forge-orchestrator`:
   ```
   /home/username/.forge-orchestrator/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
