...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".forge-orchestrator");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.FORGE_ORCH_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$FORGE_ORCH_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via FORGE_ORCH_STATE_DIR.
