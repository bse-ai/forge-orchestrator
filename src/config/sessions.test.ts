...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-forge-orchestrator",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-forge-orchestrator#general");
  });
  ...
  it("derives session transcripts dir from FORGE_ORCH_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { FORGE_ORCH_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
