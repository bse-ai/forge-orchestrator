...
const ETC_FORGE_ORCH_DIR = path.join(ROOT_DIR, "etc", "forge-orchestrator");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "forge-orchestrator.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcForgeOrchestratorPath(...parts: string[]) {
  return path.join(ETC_FORGE_ORCH_DIR, ...parts);
}
