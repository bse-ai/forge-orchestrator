...
function validateIdentityAvatar(config: ForgeOrchestratorConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: ForgeOrchestratorConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = ForgeOrchestratorSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as ForgeOrchestratorConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as ForgeOrchestratorConfig);
  ...
  return {
    ok: true,
    config: validated.data as ForgeOrchestratorConfig,
  };
}
