...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "forge-orchestrator-config-"));
  try {
    await run(home);
  } finally {
...
async function writeConfig(
  home: string,
  dirname: ".forge-orchestrator",
  port: number,
  filename: string = "forge-orchestrator.json",
) {
...
describe("config io paths", () => {
  it("uses ~/.forge-orchestrator/forge-orchestrator.json when config exists", async () => {
  ...
  it("defaults to ~/.forge-orchestrator/forge-orchestrator.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
  ...
  it("uses FORGE_ORCH_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { FORGE_ORCH_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".forge-orchestrator", "forge-orchestrator.json"));
    });
  });

  it("honors explicit FORGE_ORCH_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
