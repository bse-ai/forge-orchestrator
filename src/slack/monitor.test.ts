...
describe("buildSlackSlashCommandMatcher", () => {
  it("matches with or without a leading slash", () => {
    const matcher = buildSlackSlashCommandMatcher("forge-orchestrator");

    expect(matcher.test("forge-orchestrator")).toBe(true);
    expect(matcher.test("/forge-orchestrator")).toBe(true);
  });
  ...
  it("does not match similar names", () => {
    ...
    expect(matcher.test("/forge-orchestrator-bot")).toBe(false);
    expect(matcher.test("forge-orchestrator-bot")).toBe(false);
  });
});
