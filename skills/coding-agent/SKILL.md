...
metadata:
  {
    "forge-orchestrator": { "emoji": "🧩", "requires": { "anyBins": ["claude", "codex", "opencode", "pi"] } },
  }
---
...
**⚠️ CRITICAL: Never review PRs in ForgeOrchestrator's own project folder!**
...
For long-running background tasks, append a wake trigger to your prompt so ForgeOrchestrator gets notified immediately when the agent finishes (instead of waiting for the next heartbeat):

```
... your task here.

When completely finished, run this command to notify me:
forge-orchestrator system event --text "Done: [brief summary of what was built]" --mode now
```
...
When completely finished, run: forge-orchestrator system event --text \"Done: Built todos REST API with CRUD endpoints\" --mode now'"
```
