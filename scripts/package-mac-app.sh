...
# Build and bundle ForgeOrchestrator into a minimal .app we can open.
# Outputs to dist/ForgeOrchestrator.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/ForgeOrchestrator.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="ForgeOrchestrator"
BUNDLE_ID="${BUNDLE_ID:-ai.forge-orchestrator.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/forge-orchestrator/forge-orchestrator/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/ForgeOrchestrator/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :ForgeOrchestratorBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :ForgeOrchestratorGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/ForgeOrchestrator"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/ForgeOrchestrator"
fi
chmod +x "$APP_ROOT/Contents/MacOS/ForgeOrchestrator"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/ForgeOrchestrator" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/ForgeOrchestrator/Resources/ForgeOrchestrator.icns" "$APP_ROOT/Contents/Resources/ForgeOrchestrator.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/ForgeOrchestrator/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying ForgeOrchestratorKit resources"
FORGEORCHKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ForgeOrchestratorKit_ForgeOrchestratorKit.bundle"
if [ -d "$FORGEORCHKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ForgeOrchestratorKit_ForgeOrchestratorKit.bundle"
  cp -R "$FORGEORCHKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ForgeOrchestratorKit_ForgeOrchestratorKit.bundle"
else
  echo "WARN: ForgeOrchestratorKit resource bundle not found at $FORGEORCHKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running ForgeOrchestrator"
killall -q ForgeOrchestrator 2>/dev/null || true
