#!/usr/bin/env bash
# Reset ForgeOrchestrator like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${FORGE_ORCH_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="ForgeOrchestrator.app/Contents/MacOS/ForgeOrchestrator"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/ForgeOrchestrator"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/ForgeOrchestrator"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/ForgeOrchestrator"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.forge-orchestrator.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/forge-orchestrator-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${FORGE_ORCH_RESTART_LOG:-/tmp/forge-orchestrator-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${FORGE_ORCH_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.forge-orchestrator/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  FORGE_ORCH_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node forge-orchestrator.mjs daemon install --force --runtime node"
      log "  node forge-orchestrator.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.forge-orchestrator/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.forge-orchestrator.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing ForgeOrchestrator instances"
kill_all_forge-orchestrator
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product ForgeOrchestrator"
...
if [ "$NO_SIGN" -eq 1 ]; then
  ...
  mkdir -p "${HOME}/.forge-orchestrator"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/ForgeOrchestrator.app" ]]; then
    APP_BUNDLE="/Applications/ForgeOrchestrator.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/ForgeOrchestrator.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/ForgeOrchestrator.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/ForgeOrchestrator.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set FORGE_ORCH_APP_BUNDLE to your installed ForgeOrchestrator.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node forge-orchestrator.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node forge-orchestrator.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".forge-orchestrator", "forge-orchestrator.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: ForgeOrchestrator is running."
else
