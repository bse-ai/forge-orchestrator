...
# Configuration
SUBSYSTEM="ai.forge-orchestrator"
DEFAULT_LEVEL="info"
...
show_usage() {
    cat << EOF
clawlog - ForgeOrchestrator Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View ForgeOrchestrator logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    ForgeOrchestrator logs flow through the macOS unified log (subsystem: ai.forge-orchestrator).
