 ...
 * `forge-orchestrator update`. Use it to verify the completion installation flow
...
const CLI_NAME = "forge-orchestrator";
...
\`forge-orchestrator update\`, \`forge-orchestrator doctor\`, and \`forge-orchestrator onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# ForgeOrchestrator Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`forge-orchestrator completion --install\` later to enable.`));
    return;
  }
