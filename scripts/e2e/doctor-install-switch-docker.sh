...
IMAGE_NAME="forge-orchestrator-doctor-install-switch-e2e"
...
docker run --rm -e COREPACK_ENABLE_DOWNLOAD_PROMPT=0 "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/forge-orchestrator-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/forge-orchestrator-bin/systemctl

  cat > /tmp/forge-orchestrator-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/forge-orchestrator-bin/loginctl
  ...
  npm install -g --prefix /tmp/npm-prefix "/app/$pkg_tgz"
	  ...
	  npm_bin="/tmp/npm-prefix/bin/forge-orchestrator"
	  npm_root="/tmp/npm-prefix/lib/node_modules/forge-orchestrator"
	  ...
	  git_cli="/app/forge-orchestrator.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/forge-orchestrator-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/forge-orchestrator-gateway.service"
    if [ ! -f "$unit_path" ]; then
