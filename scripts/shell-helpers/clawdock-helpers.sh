#!/usr/bin/env bash
# ClawDock - Docker helpers for ForgeOrchestrator
# Inspired by Simon Willison's "Running ForgeOrchestrator in Docker"
# https://til.simonwillison.net/llms/forge-orchestrator-docker
#
# Installation:
...
# Common paths to check for ForgeOrchestrator
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/forge-orchestrator"
  "${HOME}/workspace/forge-orchestrator"
  "${HOME}/projects/forge-orchestrator"
  "${HOME}/dev/forge-orchestrator"
  "${HOME}/code/forge-orchestrator"
  "${HOME}/src/forge-orchestrator"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found ForgeOrchestrator at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/forge-orchestrator"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ ForgeOrchestrator not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/forge-orchestrator/forge-orchestrator.git ~/forge-orchestrator"
    echo "  cd ~/forge-orchestrator && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^FORGE_ORCH_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d forge-orchestrator-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart forge-orchestrator-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f forge-orchestrator-gateway
}
...
clawdock-workspace() {
  cd ~/.forge-orchestrator/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec forge-orchestrator-gateway \
    bash -c 'echo "alias forge-orchestrator=\"./forge-orchestrator.mjs\"" > /tmp/.bashrc_forge-orchestrator && bash --rcfile /tmp/.bashrc_forge-orchestrator'
}
...
clawdock-cli() {
  _clawdock_compose run --rm forge-orchestrator-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build forge-orchestrator-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "FORGE_ORCH_GATEWAY_TOKEN=$token" forge-orchestrator-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" forge-orchestrator-gateway \
    bash -c './forge-orchestrator.mjs config set gateway.remote.token "$TOKEN" && ./forge-orchestrator.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec forge-orchestrator-gateway \
    bash -c "./forge-orchestrator.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart forge-orchestrator-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm forge-orchestrator-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec forge-orchestrator-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'forge-orchestrator config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for ForgeOrchestrator${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (forge-orchestrator alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to forge-orchestrator project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.forge-orchestrator)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'forge-orchestrator channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'forge-orchestrator status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.forge-orchestrator.ai${_CLR_RESET}"
  echo ""
}
