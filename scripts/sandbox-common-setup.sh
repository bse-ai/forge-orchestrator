...
BASE_IMAGE="${BASE_IMAGE:-forge-orchestrator-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-forge-orchestrator-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=forge-orchestrator.sandbox=1)
NOTE
